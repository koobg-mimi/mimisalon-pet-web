module.exports = [
  ...
  (e) => {
    ...
    var R = e.i(600220),
      ...
      x = e.i(820908)
    let A = g.z.object({
      name: g.z.string().min(1).optional(),
      type: g.z.enum(['DOG', 'CAT']).optional(),
      breedId: g.z.string().optional().nullable(),
      weight: g.z.number().positive().optional().nullable(),
      age: g.z.number().int().positive().optional().nullable(),
      birthDate: g.z.string().datetime().optional().nullable(),
      gender: g.z.enum(['MALE', 'FEMALE', 'UNKNOWN']).optional().nullable(),
      hairType: g.z.enum(['SHORT_HAIR', 'LONG_HAIR']).optional().nullable(),
      specialNeeds: g.z.string().optional().nullable(),
      vaccinationStatus: g.z.enum(['UP_TO_DATE', 'OVERDUE', 'PARTIAL', 'UNKNOWN']).optional(),
      vaccinationDate: g.z.string().datetime().optional().nullable(),
    })
    async function N(e, { params: t }) {
      try {
        let { id: e } = await t,
          r = await y.default.api.getSession({ headers: await (0, b.headers)() })
        if (!r?.user?.id) return new _.NextResponse('Unauthorized', { status: 401 })
        ...
        if (!a) return new _.NextResponse('Pet not found', { status: 404 })
        return (
          a.images &&
            a.images.length > 0 &&
            (a.images = await Promise.all(
              a.images.map(async (e) => {
                if ((0, x.isGcsUrl)(e.url)) {
          ...
          _.NextResponse.json(a)
        )
      } catch (e) {
        return (
          console.error('Failed to fetch pet:', e),
          new _.NextResponse('Internal Server Error', { status: 500 })
        )
      }
    }
    async function P(e, { params: t }) {
      try {
        let { id: r } = await t,
          a = await y.default.api.getSession({ headers: await (0, b.headers)() })
        if (!a?.user?.id) return new _.NextResponse('Unauthorized', { status: 401 })
        if (
          !(await E.prisma.pet.findFirst({ where: { id: r, customerId: a.user.id, isActive: !0 } }))
        )
          return new _.NextResponse('Pet not found', { status: 404 })
        let n = await e.json(),
          i = A.parse(n)
        'DOG' === i.type && (i.hairType = null)
        let o = await E.prisma.pet.update({
          where: { id: r },
          data: {
            ...i,
            birthDate: i.birthDate ? new Date(i.birthDate) : null === i.birthDate ? null : void 0,
            vaccinationDate: i.vaccinationDate
              ? new Date(i.vaccinationDate)
              : null === i.vaccinationDate
                ? null
                : void 0,
          },
        ...
        if (e instanceof g.z.ZodError)
          return _.NextResponse.json({ error: 'Invalid data', details: e.issues }, { status: 400 })
        return (
    ...
    async function O(e, { params: t }) {
      try {
        ...
        if (!r?.user?.id) return new _.NextResponse('Unauthorized', { status: 401 })
        if (
          !(await E.prisma.pet.findFirst({ where: { id: e, customerId: r.user.id, isActive: !0 } }))
        )
          return new _.NextResponse('Pet not found', { status: 404 })
        return (
          await E.prisma.pet.update({ where: { id: e }, data: { isActive: !1 } }),
          new _.NextResponse(null, { status: 204 })
        )
      } catch (e) {
        return (
    ...
    e.s(['DELETE', () => O, 'GET', () => N, 'PUT', () => P, 'updatePetSchema', 0, A], 5848)
    var C = e.i(5848)
    let T = new t.AppRouteRouteModule({
        definition: {
          kind: r.RouteKind.APP_ROUTE,
          page: '/api/customer/pets/[id]/route',
          pathname: '/api/customer/pets/[id]',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/customer/pets/[id]/route.ts',
        nextConfigOutput: 'standalone',
        userland: C,
      }),
      { workAsyncStorage: j, workUnitAsyncStorage: D, serverHooks: S } = T
    function U() {
      return (0, a.patchFetch)({ workAsyncStorage: j, workUnitAsyncStorage: D })
    }
    async function I(e, t, a) {
      T.isDev && (0, n.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let _ = '/api/customer/pets/[id]/route'
      _ = _.replace(/\/index$/, '') || '/'
      let b = await T.prepare(e, t, { srcPage: _, multiZoneDraftMode: !1 })
      if (!b)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        U = (0, l.normalizeAppPath)(_),
        I = !!(N.dynamicRoutes[U] || N.routes[j]),
        k = async () => (
          (null == P ? void 0 : P.render404)
            ? await P.render404(e, t, x, !1)
      ...
      if (I && !A) {
        let e = !!N.routes[j],
          t = N.dynamicRoutes[U]
        if (t && !1 === t.fallback && !e) {
          if (E.experimental.adapterPath) return await k()
          throw new w.NoFallbackError()
        }
      }
      let q = null
      !I || T.isDev || A || (q = '/index' === (q = j) ? '/' : q)
      let H = !0 === T.isDev || !I,
        F = I && !H
      S &&
        D &&
      ...
      let M = e.method || 'GET',
        ...
        K = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, a) => T.onRequestError(e, t, a, P),
          },
          sharedContext: { buildId: g },
        },
        $ = new u.NodeNextRequest(e),
        G = new u.NodeNextResponse(t),
        V = d.NextRequestAdapter.fromNodeNextRequest($, (0, d.signalFromNodeResponse)(t))
      try {
        let o = async (e) =>
            T.handle(V, K).finally(() => {
              if (!e) return
          ...
          l = async (n) => {
            ...
            let u = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!I)
                    return (await (0, h.sendResponse)($, G, i, K.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await i.blob(),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__118c4056._.js.map
