  ...
  (e) => {
    ...
    function s(e, r) {
      let s =
        ...
        a = (0, o.toDate)(e, r?.in),
        n = a.getDay()
      return (a.setDate(a.getDate() - (7 * (n < s) + n - s)), a.setHours(0, 0, 0, 0), a)
    }
  ...
  (e) => {
    ...
    function o(e, o) {
      let s = (0, t.getDefaultOptions)(),
        ...
        i = n.getDay()
      return (
        n.setDate(n.getDate() + ((i < a ? -7 : 0) + 6 - (i - a))),
        n.setHours(23, 59, 59, 999),
        n
      )
    }
    e.s(['endOfWeek', () => o], 528164)
    var s = e.i(874321)
    function a(e, t, o) {
      var a
      let n
      return (
        (a = -(7 * t)),
        (n = (0, r.toDate)(e, o?.in)),
        isNaN(a) ? (0, s.constructFrom)(o?.in || e, NaN) : (a && n.setDate(n.getDate() + a), n)
      )
    }
    e.s(['subWeeks', () => a], 458325)
  },
  905404,
  (e) => {
    'use strict'
    ...
    class r {
      ...
      static calculateSettlement(e, t, r = 0) {
        ...
        let o = e.reduce((e, t) => e + t.totalPrice, 0),
          s = (t / 100) * o,
          a = r > 0 ? (r / 100) * o : 0
        return {
          totalRevenue: o,
          commissionRate: t,
          platformCommission: s,
          netSettlementAmount: o - s - a,
          taxAmount: a,
      ...
      static async fetchBookingsForGroomers(e, r, o) {
        let s = await t.prisma.booking.findMany({
            where: {
              groomerId: { in: e },
              status: 'SERVICE_COMPLETED',
              completedAt: { gte: r, lte: o },
            },
            select: {
              id: !0,
              bookingNumber: !0,
      ...
      static async checkExistingSettlements(e, r, o) {
        return new Set(
          (
            await t.prisma.groomerSettlement.findMany({
              where: { groomerId: { in: e }, periodStartDate: r, periodEndDate: o },
              select: { groomerId: !0 },
            })
          ).map((e) => e.groomerId)
        )
      }
      static async createSettlement(e, r, o, s, a, n) {
        return await t.prisma.$transaction(async (t) => {
          let i = await t.groomerSettlement.create({
            data: {
              ...
              netSettlementAmount: o.netSettlementAmount,
              status: 'CALCULATED',
          ...
          for (let s of r) {
            let r = s.totalPrice * (o.commissionRate / 100),
              a = o.taxAmount > 0 ? s.totalPrice * (o.taxAmount / o.totalRevenue) : 0,
              n = s.totalPrice - r - a
            await t.groomerSettlementDetail.create({
              data: {
                ...
                groomerProfileId: e.groomerProfile.id,
                bookingId: s.id,
                bookingDate: s.completedAt,
                serviceAmount: s.totalPrice,
                commissionRate: o.commissionRate,
                commissionAmount: r,
                taxAmount: a,
                netAmount: n,
      ...
      static async processSettlements(e, t, r = {}) {
        let { skipExisting: o = !0, dryRun: s = !1, onProgress: a } = r,
          n = await this.fetchActiveGroomers(),
          i = [],
          l = 0
        a?.({ completed: 0, total: n.length, current: 'Fetching groomer data...' })
        let c = n.map((e) => e.id),
          u = o ? await this.checkExistingSettlements(c, e, t) : new Set(),
          d = await this.fetchBookingsForGroomers(c, e, t)
        a?.({ completed: 0, total: n.length, current: 'Processing settlements...' })
        let m = 0
        for (let r of n) {
          try {
            let o
            ...
            let f = this.calculateSettlement(
              p,
              r.groomerProfile.commissionGrade.commissionRate,
              r.groomerProfile.taxRate
            )
            ;(s || (o = await this.createSettlement(r, p, f, e, t, '배치 자동 정산')),
              i.push({
                groomerId: r.id,
                groomerName: r.name || '알 수 없음',
                status: 'success',
        ...
        a?.({ completed: m, total: n.length, current: 'Complete!' })
        let p = {
          total: i.length,
  ...
  (e) => {
    ...
    async function x(e) {
      try {
        ...
        try {
          if (
            ((t = (await e.json().catch(() => ({}))).weekOffset ?? 1),
            'number' != typeof t || t < 0 || t > 52)
          )
            return w.NextResponse.json(
              { error: 'weekOffset은 0부터 52 사이의 숫자여야 합니다', received: t },
              { status: 400 }
            )
          let s = (0, E.subWeeks)(new Date(), t)
          ;((r = (0, S.startOfWeek)(s, { weekStartsOn: 1 })),
            (o = (0, A.endOfWeek)(s, { weekStartsOn: 1 })))
        } catch (e) {
          return w.NextResponse.json(
            {
        ...
        try {
          let e = await I.SettlementCalculator.processSettlements(r, o, {
            skipExisting: !0,
            dryRun: !1,
            onProgress: (e) => {
              console.log(`정산 진행률: ${e.completed}/${e.total} (${e.current})`)
            },
          })
          ;((s = e.results), (a = e.summary))
        } catch (e) {
          return (
            ...
            w.NextResponse.json(
              {
                ...
                details: e instanceof Error ? e.message : '알 수 없는 오류',
                period: { start: r.toISOString(), end: o.toISOString() },
              },
              { status: 500 }
            )
          )
        ...
        let i = s
            ...
            .map((e) => {
              try {
                return {
                  ...
                  bookingCount: e.calculation?.bookingCount || 0,
                  totalRevenue: e.calculation?.totalRevenue || 0,
                  platformCommission: e.calculation?.platformCommission || 0,
                  netAmount: e.calculation?.netSettlementAmount || 0,
                  status: 'success',
                }
              } catch (t) {
                return (
                  console.error(`Error processing successful result for ${e.groomerId}:`, t),
                  null
                )
              }
            })
          ...
          c = s
            ...
            .map((e) => ({
              ...
              groomerName: e.groomerName,
              reason: 'failed',
              error: e.error || '알 수 없는 오류',
            })),
          u = {
            activeGroomers: [],
            results: i,
            skippedGroomers: [...l, ...c],
            totalCreated: a.successful,
            totalAmount: a.totalAmount,
          }
        if (
          (a.failed > 0 || a.skipped > 0) &&
          (console.warn(
            `정산 처리 완료 - 성공: ${a.successful}, 실패: ${a.failed}, 건너뜀: ${a.skipped}`
          ),
          a.failed > 0)
        ) {
          let e = c.map((e) => `${e.groomerName}(${e.groomerId}): ${e.error}`).join(', ')
          console.error(`실패한 정산들: ${e}`)
        }
        let d = a.failed > 0,
          m = a.successful > 0,
          p = m && (d || a.skipped > 0),
          f = 200,
          g = `${(0, C.format)(r, 'yyyy-MM-dd', { locale: R.ko })} ~ ${(0, C.format)(o, 'yyyy-MM-dd', { locale: R.ko })} 기간 정산이 생성되었습니다`
        return (
          !m && d
            ? ((f = 500), (g = '모든 정산 처리가 실패했습니다'))
            : p &&
              ((f = 207),
              (g = `부분 성공: ${a.successful}개 성공, ${a.failed}개 실패, ${a.skipped}개 건너뜀`)),
          w.NextResponse.json(
            {
              success: m,
              partialSuccess: p,
              ...
              summary: {
                period: { start: r.toISOString(), end: o.toISOString() },
                totalGroomers: a.total,
                totalCreated: u.totalCreated,
                totalSkipped: u.skippedGroomers.length,
                totalAmount: u.totalAmount,
                failed: a.failed,
                skippedReasons: {
                  noCommissionRate: s.filter(
                    (e) => 'no_commission_rate' === e.reason || 'no_commission_grade' === e.reason
                  ).length,
                  alreadyExists: s.filter((e) => 'already_exists' === e.reason).length,
                  noBookings: s.filter((e) => 'no_bookings' === e.reason).length,
                  belowMinimum: s.filter((e) => 'below_minimum' === e.reason).length,
                  noProfile: s.filter((e) => 'no_profile' === e.reason).length,
                  systemErrors: a.failed,
                },
              },
              results: u.results,
              skippedGroomers: u.skippedGroomers,
              errors: d ? c : void 0,
            },
            { status: f }
          )
        ...
        return (
          console.error('Critical error in weekly settlement creation:', e),
          'development' === b.env.NODE_ENV && console.error('Stack trace:', e),
          w.NextResponse.json(
            {
    ...
    let N = new t.AppRouteRouteModule({
        definition: {
          ...
          page: '/api/admin/settlements/create-weekly/route',
          pathname: '/api/admin/settlements/create-weekly',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/admin/settlements/create-weekly/route.ts',
        nextConfigOutput: 'standalone',
        userland: P,
      }),
      { workAsyncStorage: O, workUnitAsyncStorage: D, serverHooks: j } = N
    function M() {
      return (0, o.patchFetch)({ workAsyncStorage: O, workUnitAsyncStorage: D })
    }
    async function $(e, t, o) {
      N.isDev && (0, s.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let R = '/api/admin/settlements/create-weekly/route'
      R = R.replace(/\/index$/, '') || '/'
      let w = await N.prepare(e, t, { srcPage: R, multiZoneDraftMode: !1 })
      if (!w)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        M = !!(C.dynamicRoutes[j] || C.routes[P]),
        $ = async () => (
      ...
      if (M && !E) {
        let e = !!C.routes[P],
          t = C.dynamicRoutes[j]
        if (t && !1 === t.fallback && !e) {
          if (S.experimental.adapterPath) return await $()
          throw new k.NoFallbackError()
        }
      }
      let T = null
      !M || N.isDev || E || (T = '/index' === (T = P) ? '/' : T)
      let G = !0 === N.isDev || !M,
        H = M && !G
      D &&
        O &&
      ...
      let q = e.method || 'GET',
        ...
        V = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, o) => N.onRequestError(e, t, o, I),
          },
          sharedContext: { buildId: _ },
        },
        B = new c.NodeNextRequest(e),
        W = new c.NodeNextResponse(t),
        K = u.NextRequestAdapter.fromNodeNextRequest(B, (0, u.signalFromNodeResponse)(t))
      try {
        let n = async (e) =>
            N.handle(K, V).finally(() => {
              if (!e) return
          ...
          l = async (s) => {
            ...
            let c = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!M)
                    return (await (0, p.sendResponse)(B, W, a, V.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await a.blob(),
            ...
            ;(i ||
              t.setHeader(
                'x-nextjs-cache',
                b ? 'REVALIDATED' : u.isMiss ? 'MISS' : u.isStale ? 'STALE' : 'HIT'
              ),
              E &&
                t.setHeader(
                  'Cache-Control',
            ...
            return (
              (i && M) || d.delete(h.NEXT_CACHE_TAGS_HEADER),
              !u.cacheControl ||
        ...
        F
          ? await l(F)
          : await U.withPropagatedContext(e.headers, () =>
              U.trace(
        ...
        if (
          ...
          throw t
        return (await (0, p.sendResponse)(B, W, new Response(null, { status: 500 })), null)
      }
    }
    e.s(
      [
