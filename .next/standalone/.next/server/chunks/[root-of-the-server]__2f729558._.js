module.exports = [
  ...
  (e) => {
    ...
    async function P() {
      try {
        let e = await f.default.api.getSession({ headers: await (0, w.headers)() })
        if (!e || e.user?.role !== 'ADMIN')
          return E.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        let [t, a, r, n] = await Promise.all([
            C.prisma.user.count(),
            C.prisma.user.count({ where: { role: 'CUSTOMER' } }),
            C.prisma.user.count({ where: { role: 'GROOMER' } }),
            C.prisma.user.count({ where: { role: 'ADMIN' } }),
          ]),
          [s, o, i, l] = await Promise.all([
            ...
            C.prisma.booking.count({ where: { status: 'SERVICE_COMPLETED' } }),
            C.prisma.booking.count({ where: { status: 'SERVICE_CANCELLED' } }),
          ]),
          u = new Date()
        u.setHours(0, 0, 0, 0)
        let d = new Date(u)
        ...
        let [c, p] = await Promise.all([
            C.prisma.booking.count({ where: { createdAt: { gte: u, lt: d } } }),
            C.prisma.payment.aggregate({
              where: { createdAt: { gte: u, lt: d }, status: 'COMPLETED' },
              _sum: { amount: !0 },
            }),
          ]),
          h = new Date(u.getFullYear(), u.getMonth(), 1),
          m = await C.prisma.payment.aggregate({
            where: { createdAt: { gte: h }, status: 'COMPLETED' },
            _sum: { amount: !0 },
          }),
          _ = await C.prisma.payment.aggregate({
            where: { status: 'COMPLETED' },
            _sum: { amount: !0 },
          }),
          [R, v] = await Promise.all([
            C.prisma.review.count(),
            C.prisma.review.aggregate({ _avg: { rating: !0 } }),
          ]),
          g = {
            totalUsers: t,
            totalCustomers: a,
            ...
            cancelledBookings: l,
            todayBookings: c,
            todayRevenue: p._sum.amount || 0,
            monthlyRevenue: m._sum.amount || 0,
            totalRevenue: _._sum.amount || 0,
            totalReviews: R,
            averageRating: v._avg.rating || 0,
          }
        return E.NextResponse.json(g)
      } catch (e) {
        return (
    ...
    let N = new t.AppRouteRouteModule({
      ...
      { workAsyncStorage: b, workUnitAsyncStorage: x, serverHooks: T } = N
    function y() {
      return (0, r.patchFetch)({ workAsyncStorage: b, workUnitAsyncStorage: x })
    }
    async function O(e, t, r) {
      N.isDev && (0, n.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let E = '/api/admin/stats/route'
      E = E.replace(/\/index$/, '') || '/'
      let w = await N.prepare(e, t, { srcPage: E, multiZoneDraftMode: !1 })
      if (!w)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        S = !!(x.dynamicRoutes[I] || x.routes[k]),
        j = async () => (
      ...
      if (S && !b) {
        let e = !!x.routes[k],
          t = x.dynamicRoutes[I]
        if (t && !1 === t.fallback && !e) {
          if (P.experimental.adapterPath) return await j()
          throw new v.NoFallbackError()
        }
      }
      let H = null
      !S || N.isDev || b || (H = '/index' === (H = k) ? '/' : H)
      let U = !0 === N.isDev || !S,
        q = S && !U
      M &&
        D &&
      ...
      let F = e.method || 'GET',
        ...
        $ = {
          ...
          prerenderManifest: x,
          renderOpts: {
            experimental: { authInterrupts: !!P.experimental.authInterrupts },
            ...
            onInstrumentationRequestError: (t, a, r) => N.onRequestError(e, t, r, T),
          },
          sharedContext: { buildId: f },
        },
        K = new u.NodeNextRequest(e),
        V = new u.NodeNextResponse(t),
        B = d.NextRequestAdapter.fromNodeNextRequest(K, (0, d.signalFromNodeResponse)(t))
      try {
        let o = async (e) =>
            N.handle(B, $).finally(() => {
              if (!e) return
              e.setAttributes({ 'http.status_code': t.statusCode, 'next.rsc': !1 })
              let a = L.getRootSpanAttributes()
              if (!a) return
          ...
          l = async (n) => {
            ...
            let u = async ({ previousCacheEntry: a }) => {
                try {
                  ...
                  if (!S)
                    return (await (0, h.sendResponse)(K, V, s, $.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await s.blob(),
            ...
            ;(i ||
              t.setHeader(
                'x-nextjs-cache',
                y ? 'REVALIDATED' : d.isMiss ? 'MISS' : d.isStale ? 'STALE' : 'HIT'
              ),
              b &&
                t.setHeader(
                  'Cache-Control',
            ...
            return (
              ...
              !d.cacheControl ||
                ...
                c.get('Cache-Control') ||
                c.set('Cache-Control', (0, _.getCacheControlHeader)(d.cacheControl)),
        ...
        if (
          ...
          throw t
        return (await (0, h.sendResponse)(K, V, new Response(null, { status: 500 })), null)
      }
    }
    e.s(
      [
...
//# sourceMappingURL=%5Broot-of-the-server%5D__2f729558._.js.map
