module.exports = [
    ...
    async function t(e) {
      ...
      try {
        let r = encodeURIComponent(e),
          s = `https://dapi.kakao.com/v2/local/search/address.json?query=${r}`
  ...
  (e) => {
    ...
    var _ = e.i(600220),
      ...
      x = e.i(125289)
    let b = A.z.object({
      street: A.z.string().min(1).optional(),
      city: A.z.string().min(1).optional(),
      state: A.z.string().min(1).optional(),
      zipCode: A.z.string().min(1).optional(),
      country: A.z.string().optional(),
      isDefault: A.z.boolean().optional(),
      detailAddress: A.z.string().optional(),
    })
    async function C(e, { params: t }) {
      try {
        let { id: r } = await t,
          s = await y.default.api.getSession({ headers: await (0, w.headers)() })
        if (!s?.user?.email) return g.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        if ('CUSTOMER' !== s.user.role)
          return g.NextResponse.json({ error: 'Forbidden' }, { status: 403 })
        let a = await E.prisma.user.findUnique({ where: { email: s.user.email } })
        if (!a) return g.NextResponse.json({ error: 'User not found' }, { status: 404 })
        let o = await E.prisma.address.findFirst({ where: { id: r, customerId: a.id } })
        if (!o) return g.NextResponse.json({ error: 'Address not found' }, { status: 404 })
        let n = await e.json(),
          i = b.parse(n)
        i.isDefault &&
          !o.isDefault &&
          (await E.prisma.address.updateMany({
            where: { customerId: a.id, isDefault: !0, id: { not: r } },
            data: { isDefault: !1 },
          }))
        let d = {}
        if (i.street) {
          let e = `${i.street}${i.detailAddress ? ` ${i.detailAddress}` : ''}`
          d.street = e
          try {
            let t = await (0, x.geocodeAddress)(e)
            t && ((d.centerLat = t.latitude), (d.centerLng = t.longitude))
          } catch (e) {
            console.warn('Failed to geocode updated customer address:', e)
          }
        }
        ;(i.city && (d.city = i.city),
          i.state && (d.state = i.state),
          i.zipCode && (d.zipCode = i.zipCode),
          i.country && (d.country = i.country),
          void 0 !== i.isDefault && (d.isDefault = i.isDefault))
        let l = await E.prisma.address.update({ where: { id: r }, data: d })
        return g.NextResponse.json(l)
      } catch (e) {
        if (e instanceof A.z.ZodError)
          return g.NextResponse.json({ error: 'Invalid data', details: e.issues }, { status: 400 })
        return (
    ...
    async function N(e, { params: t }) {
      try {
        let { id: e } = await t,
          r = await y.default.api.getSession({ headers: await (0, w.headers)() })
        if (!r?.user?.email) return g.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        if ('CUSTOMER' !== r.user.role)
          return g.NextResponse.json({ error: 'Forbidden' }, { status: 403 })
        let s = await E.prisma.user.findUnique({ where: { email: r.user.email } })
        if (!s) return g.NextResponse.json({ error: 'User not found' }, { status: 404 })
        let a = await E.prisma.address.findFirst({ where: { id: e, customerId: s.id } })
        if (!a) return g.NextResponse.json({ error: 'Address not found' }, { status: 404 })
        if (a.isDefault) {
          let t = await E.prisma.address.findFirst({
            where: { customerId: s.id, id: { not: e } },
            orderBy: { createdAt: 'desc' },
          })
          t && (await E.prisma.address.update({ where: { id: t.id }, data: { isDefault: !0 } }))
        }
        return (
          await E.prisma.address.delete({ where: { id: e } }),
          g.NextResponse.json({ message: 'Address deleted successfully' })
        )
      } catch (e) {
        return (
    ...
    e.s(['DELETE', () => N, 'PUT', () => C, 'updateAddressSchema', 0, b], 446327)
    var j = e.i(446327)
    let P = new t.AppRouteRouteModule({
        definition: {
          kind: r.RouteKind.APP_ROUTE,
          page: '/api/customer/addresses/[id]/route',
          pathname: '/api/customer/addresses/[id]',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/customer/addresses/[id]/route.ts',
        nextConfigOutput: 'standalone',
        userland: j,
      }),
      { workAsyncStorage: k, workUnitAsyncStorage: T, serverHooks: S } = P
    function O() {
      return (0, s.patchFetch)({ workAsyncStorage: k, workUnitAsyncStorage: T })
    }
    async function I(e, t, s) {
      P.isDev && (0, a.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let g = '/api/customer/addresses/[id]/route'
      g = g.replace(/\/index$/, '') || '/'
      ...
      let {
        ...
        I = (0, d.normalizeAppPath)(g),
        U = !!(C.dynamicRoutes[I] || C.routes[T]),
        D = async () => (
          (null == N ? void 0 : N.render404)
            ? await N.render404(e, t, x, !1)
      ...
      let z = e.method || 'GET',
        ...
        $ = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, s) => P.onRequestError(e, t, s, N),
          },
      ...
      try {
          ...
          d = async (a) => {
            ...
            let l = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  d && s.waitUntil && (s.waitUntil(d), (d = void 0))
                  let l = $.renderOpts.collectedTags
                  if (!U)
                    return (await (0, h.sendResponse)(L, B, o, $.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await o.blob(),
                    ...
                    let r =
                        ...
                        !($.renderOpts.collectedRevalidate >= R.INFINITE_CACHE) &&
                        $.renderOpts.collectedRevalidate,
                      s =
                        void 0 === $.renderOpts.collectedExpire ||
                        $.renderOpts.collectedExpire >= R.INFINITE_CACHE
                          ? void 0
        ...
        if (
          ...
          throw t
        return (await (0, h.sendResponse)(L, B, new Response(null, { status: 500 })), null)
      }
    }
    e.s(
      [
...
//# sourceMappingURL=%5Broot-of-the-server%5D__61ef3198._.js.map
