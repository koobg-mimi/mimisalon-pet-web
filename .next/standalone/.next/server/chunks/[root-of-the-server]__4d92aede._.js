module.exports = [
  ...
  (e) => {
    ...
    async function E(e) {
      try {
        let t = await y.default.api.getSession({ headers: await (0, b.headers)() })
        ...
        let r = e.nextUrl.searchParams,
          a = parseInt(r.get('page') || '1'),
          n = parseInt(r.get('limit') || '20'),
          s = r.get('status'),
          o = await A.prisma.user.findUnique({ where: { email: t.user.email } })
        if (!o) return g.NextResponse.json({ error: 'User not found' }, { status: 404 })
        let i = { customerId: o.id }
        if (s && 'ALL' !== s) {
          let e = s.split(',').map((e) => e.trim())
          e.length > 1 ? (i.status = { in: e }) : (i.status = e[0])
        }
        let l = await A.prisma.payment.count({ where: i }),
          d = (
            await A.prisma.payment.findMany({
              where: i,
              include: {
                booking: {
                  include: {
                    bookingPets: {
                      include: {
                        pet: { select: { id: !0, name: !0, type: !0, breed: !0 } },
            ...
            let t = e.booking?.bookingPets[0],
              r = t?.services[0]?.service
            return {
              id: e.id,
              ...
              booking: e.booking
                ? {
                    ...
                    serviceDate: e.booking.serviceDate,
                    serviceTime: e.booking.serviceTime,
        ...
        return g.NextResponse.json({
          payments: d,
          totalPayments: l,
          totalPages: Math.ceil(l / n),
          page: a,
          limit: n,
        })
      } catch (e) {
        return (
          console.error('Customer payments error:', e),
    ...
    let k = new t.AppRouteRouteModule({
        definition: {
          ...
          page: '/api/customer/payments/route',
          pathname: '/api/customer/payments',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/customer/payments/route.ts',
        nextConfigOutput: 'standalone',
        userland: w,
      }),
      { workAsyncStorage: x, workUnitAsyncStorage: C, serverHooks: N } = k
    function P() {
      return (0, a.patchFetch)({ workAsyncStorage: x, workUnitAsyncStorage: C })
    }
    async function T(e, t, a) {
      k.isDev && (0, n.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let g = '/api/customer/payments/route'
      g = g.replace(/\/index$/, '') || '/'
      let b = await k.prepare(e, t, { srcPage: g, multiZoneDraftMode: !1 })
      if (!b)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        I = !!(C.dynamicRoutes[S] || C.routes[j]),
        M = async () => (
      ...
      let F = e.method || 'GET',
        ...
        L = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, a) => k.onRequestError(e, t, a, N),
          },
          ...
          l = async (n) => {
            ...
            let d = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!I)
                    return (await (0, h.sendResponse)(B, G, s, L.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await s.blob(),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__4d92aede._.js.map
