module.exports = [
  ...
  (e) => {
    ...
    async function t(e) {
      ...
      try {
        ...
        let n = await a.json()
        if (0 === n.documents.length)
          return (console.warn('No geocoding results found for address:', e), null)
        let o = n.documents[0]
        return { latitude: parseFloat(o.y), longitude: parseFloat(o.x), address: o.address_name }
      } catch (e) {
  ...
  (e) => {
    ...
    var _ = e.i(600220),
      ...
      y = e.i(125289)
    let b = A.z.object({
      street: A.z.string().min(1, 'Street is required'),
      city: A.z.string().min(1, 'City is required'),
      state: A.z.string().min(1, 'State is required'),
      zipCode: A.z.string().min(1, 'Zip code is required'),
      country: A.z.string().default('KR'),
      isDefault: A.z.boolean().default(!1),
      detailAddress: A.z.string().optional(),
    })
    async function C() {
      try {
        let e = await E.default.api.getSession({ headers: await (0, w.headers)() })
        if (!e?.user?.email) return g.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        if ('CUSTOMER' !== e.user.role)
          return g.NextResponse.json({ error: 'Forbidden' }, { status: 403 })
        let t = await x.prisma.user.findUnique({ where: { email: e.user.email } })
        if (!t) return g.NextResponse.json({ error: 'User not found' }, { status: 404 })
        let r = await x.prisma.address.findMany({
    ...
    async function N(e) {
      try {
        ...
        if ('CUSTOMER' !== t.user.role)
          return g.NextResponse.json({ error: 'Forbidden' }, { status: 403 })
        let r = await x.prisma.user.findUnique({ where: { email: t.user.email } })
        if (!r) return g.NextResponse.json({ error: 'User not found' }, { status: 404 })
        let s = await e.json(),
          a = b.safeParse(s)
        if (!a.success)
          return g.NextResponse.json({ error: 'Invalid data', details: a.error }, { status: 400 })
        let {
          street: n,
          city: o,
          state: i,
        ...
        u &&
          (await x.prisma.address.updateMany({
            where: { customerId: r.id, isDefault: !0 },
            data: { isDefault: !1 },
          }))
        let p = await x.prisma.address.count({ where: { customerId: r.id } }),
          h = `${n}${c ? ` ${c}` : ''}`,
          m = null,
          f = null
        try {
          let e = await (0, y.geocodeAddress)(h)
          e && ((m = e.latitude), (f = e.longitude))
        } catch (e) {
          console.warn('Failed to geocode customer address:', e)
        }
        let R = await x.prisma.address.create({
          data: {
            street: h,
            city: o,
            state: i,
        ...
        return g.NextResponse.json(R, { status: 201 })
      } catch (e) {
    ...
    e.s(['GET', () => C, 'POST', () => N, 'createAddressSchema', 0, b], 625611)
    var P = e.i(625611)
    let j = new t.AppRouteRouteModule({
        definition: {
          kind: r.RouteKind.APP_ROUTE,
          page: '/api/customer/addresses/route',
          pathname: '/api/customer/addresses',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/customer/addresses/route.ts',
        nextConfigOutput: 'standalone',
        userland: P,
      }),
      { workAsyncStorage: k, workUnitAsyncStorage: S, serverHooks: T } = j
    function O() {
      return (0, s.patchFetch)({ workAsyncStorage: k, workUnitAsyncStorage: S })
    }
    async function I(e, t, s) {
      j.isDev && (0, a.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let g = '/api/customer/addresses/route'
      g = g.replace(/\/index$/, '') || '/'
      let w = await j.prepare(e, t, { srcPage: g, multiZoneDraftMode: !1 })
      if (!w)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        U = !!(C.dynamicRoutes[I] || C.routes[S]),
        q = async () => (
      ...
      let H = e.method || 'GET',
        ...
        F = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, s) => j.onRequestError(e, t, s, N),
          },
          ...
          l = async (a) => {
            ...
            let d = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!U)
                    return (await (0, h.sendResponse)(L, B, n, F.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await n.blob(),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a8669e82._.js.map
