  ...
  (e) => {
    ...
    async function x(e, { params: t }) {
      try {
        ...
        if (!e?.user?.email) return y.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        if ('CUSTOMER' !== e.user.role)
          return y.NextResponse.json({ error: 'Forbidden' }, { status: 403 })
        let { id: r } = await t,
          n = await k.prisma.user.findUnique({ where: { email: e.user.email } })
        if (!n) return y.NextResponse.json({ error: 'User not found' }, { status: 404 })
        let a = await k.prisma.booking.findUnique({
          where: { id: r, customerId: n.id },
          include: {
            groomer: { select: { id: !0, name: !0 } },
            customerAddress: !0,
            bookingPets: {
              include: { pet: { include: { breed: !0 } }, services: { include: { service: !0 } } },
            },
          },
        })
        if (!a) return y.NextResponse.json({ error: 'Booking not found' }, { status: 404 })
        if ('ADDITIONAL_PAYMENT_PENDING' !== a.status)
        ...
        let s = [
          {
            id: `${a.id}-additional-1`,
            name: '추가 서비스',
            description: '미용 중 추가로 필요한 서비스',
            amount: a.additionalCharges,
            quantity: 1,
            total: a.additionalCharges,
          },
        ]
        if (a.notes)
          try {
            let e = JSON.parse(a.notes)
            e.services &&
              Array.isArray(e.services) &&
              ((s.length = 0),
              e.services.forEach((e, t) => {
                s.push({
                  id: `${a.id}-additional-${t + 1}`,
                  name: e.name,
                  description: e.description || '',
                  amount: e.price,
                  quantity: e.quantity || 1,
                  total: e.price * (e.quantity || 1),
                })
              }))
          } catch (e) {
        ...
        let o = {
          id: a.id,
          status: a.status,
          originalAmount: a.basePrice,
          service: { name: a.bookingPets?.[0]?.services?.[0]?.service?.name || a.serviceType },
          pet: {
            name: a.bookingPets?.[0]?.pet?.name || 'Unknown Pet',
            breed:
              a.bookingPets?.[0]?.pet?.breed?.name ||
              a.bookingPets?.[0]?.pet?.type ||
              'Unknown Breed',
          },
          groomer: { name: a.groomer?.name || 'Unassigned' },
          location: { name: '미미살롱', address: a.customerAddress?.street || '주소 미확인' },
          date: (0, E.format)(a.serviceDate, 'yyyy-MM-dd', { locale: _.ko }),
          time: a.serviceTime,
          additionalCharges: s,
        }
        return y.NextResponse.json(o)
      } catch (e) {
        return (
          console.error('Additional payment API error:', e),
    ...
    let C = new t.AppRouteRouteModule({
        definition: {
          ...
          page: '/api/bookings/[id]/additional-payment/route',
          pathname: '/api/bookings/[id]/additional-payment',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/bookings/[id]/additional-payment/route.ts',
        nextConfigOutput: 'standalone',
        userland: A,
      }),
      { workAsyncStorage: P, workUnitAsyncStorage: N, serverHooks: O } = C
    function S() {
      return (0, n.patchFetch)({ workAsyncStorage: P, workUnitAsyncStorage: N })
    }
    async function T(e, t, n) {
      C.isDev && (0, a.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let _ = '/api/bookings/[id]/additional-payment/route'
      _ = _.replace(/\/index$/, '') || '/'
      let y = await C.prepare(e, t, { srcPage: _, multiZoneDraftMode: !1 })
      if (!y)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        I = !!(A.dynamicRoutes[D] || A.routes[S]),
        q = async () => (
      ...
      if (I && !x) {
        let e = !!A.routes[S],
          t = A.dynamicRoutes[D]
        if (t && !1 === t.fallback && !e) {
          if (k.experimental.adapterPath) return await q()
          throw new R.NoFallbackError()
        }
      }
      let U = null
      !I || C.isDev || x || (U = '/index' === (U = S) ? '/' : U)
      let M = !0 === C.isDev || !I,
        H = I && !M
      j &&
        T &&
      ...
      let F = e.method || 'GET',
        ...
        W = u.NextRequestAdapter.fromNodeNextRequest(L, (0, u.signalFromNodeResponse)(t))
      try {
        let o = async (e) =>
            C.handle(W, B).finally(() => {
              if (!e) return
          ...
          l = async (a) => {
            ...
            if (!I) return null
            if (
              (null == u || null == (s = u.value) ? void 0 : s.kind) !== g.CachedRouteKind.APP_ROUTE
            )
...
//# sourceMappingURL=%5Broot-of-the-server%5D__4810c018._.js.map
