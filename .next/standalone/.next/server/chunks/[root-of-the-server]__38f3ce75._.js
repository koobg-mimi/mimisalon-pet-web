module.exports = [
        ...
        for (let e of a)
          for (let t of e.booking.bookingPets)
            for (let r of t.services) {
              let t = r.serviceId
              ;(i.has(t) || i.set(t, []), i.get(t).push(e.rating))
            }
        for (let t of e) {
          let e = i.get(t)
          if (e && e.length > 0) {
  ...
  (e) => {
    ...
    let C = A.z.object({
      petType: A.z.string(),
      minWeight: A.z.number().optional(),
      maxWeight: A.z.number().optional(),
      price: A.z.number(),
      selectedBreedIds: A.z.array(A.z.string()).optional(),
    })
    async function S() {
      try {
        let e = await b.default.api.getSession({ headers: await (0, _.headers)() })
        if (!e || 'ADMIN' !== e.user.role)
          return y.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        let t = await w.prisma.service.findMany({
            include: {
              servicePetTypes: !0,
          ...
          r = t.map((e) => e.id),
          a = await (0, E.calculateMultipleServiceRatings)(r),
          i = t.map((e) => ({
            id: e.id,
            name: e.name,
            ...
            breedCategories: e.serviceBreedCategories.map((e) => e.breedCategory),
            priceRanges: e.priceRanges.map((e) => ({
              ...e,
              selectedBreedIds: e.applicableBreeds?.map((e) => e.breedId) || [],
            })),
            requirements: e.requiresVaccination ? '예방접종 필요' : void 0,
            afterCareInstructions: void 0,
            createdAt: e.createdAt.toISOString(),
            updatedAt: e.updatedAt.toISOString(),
            bookingCount: e._count.bookingServices,
            averageRating: a.get(e.id) ?? 0,
          }))
        return y.NextResponse.json(i)
      } catch (e) {
        return (
      ...
      try {
        ...
        if (
          (console.log(
            'Session:',
            t
              ? { user: { id: t.user?.id, email: t.user?.email, role: t.user?.role } }
              : 'No session'
          ),
          !t)
        )
          return (
            ...
            y.NextResponse.json({ error: 'Authentication required' }, { status: 401 })
          )
        if ('ADMIN' !== t.user.role)
          return (
            console.log('User role is not ADMIN:', t.user.role),
            y.NextResponse.json({ error: 'Admin access required' }, { status: 403 })
          )
        let r = await e.json()
        console.log('Received payload:', JSON.stringify(r, null, 2))
        let {
          name: a,
          description: i,
          duration: n,
        ...
        let u = await w.prisma.$transaction(async (e) => {
            let t = await e.service.create({
              data: {
                name: a,
                description: i,
                durationMinutes: n,
                requiresVaccination: !!c && c.includes('예방접종'),
                isActive: d,
              },
            })
            if (
              (o &&
                ...
                (await e.servicePetType.createMany({
                  data: o.map((e) => ({ serviceId: t.id, petType: e })),
                })),
              l &&
                l.length > 0 &&
                (await e.serviceBreedCategory.createMany({
                  data: l.map((e) => ({ serviceId: t.id, breedCategory: e })),
                })),
              s.length > 0)
            )
              for (let r of s) {
                let a = await e.servicePriceRange.create({
                  data: {
                    serviceId: t.id,
                    petType: r.petType,
                ...
                r.selectedBreedIds &&
                  r.selectedBreedIds.length > 0 &&
                  (await e.servicePriceBreed.createMany({
                    data: r.selectedBreedIds.map((e) => ({
                      servicePriceRangeId: a.id,
                      breedId: e,
                    })),
                  }))
              }
          ...
          p = await w.prisma.service.findUnique({
            where: { id: u.id },
            include: {
              servicePetTypes: !0,
              serviceBreedCategories: !0,
        ...
        return y.NextResponse.json(g, { status: 201 })
      } catch (e) {
        return (
    ...
    ;(A.z.object({
      name: A.z.string().min(1),
      description: A.z.string().optional(),
      duration: A.z.number().positive(),
      priceRanges: A.z.array(C).min(1),
      petTypes: A.z.array(A.z.string()),
      breedCategories: A.z.array(A.z.string()).optional(),
      requirements: A.z.string().optional(),
      isActive: A.z.boolean(),
    }),
      e.s(['GET', () => S, 'POST', () => x], 676369))
    var k = e.i(676369)
    let P = new t.AppRouteRouteModule({
      ...
      { workAsyncStorage: T, workUnitAsyncStorage: I, serverHooks: N } = P
    function M() {
      return (0, a.patchFetch)({ workAsyncStorage: T, workUnitAsyncStorage: I })
    }
    async function j(e, t, a) {
      P.isDev && (0, i.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let y = '/api/admin/services/route'
      y = y.replace(/\/index$/, '') || '/'
      let _ = await P.prepare(e, t, { srcPage: y, multiZoneDraftMode: !1 })
      if (!_)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        j = (0, l.normalizeAppPath)(y),
        O = !!(S.dynamicRoutes[j] || S.routes[I]),
        q = async () => (
          (null == x ? void 0 : x.render404)
            ? await x.render404(e, t, A, !1)
      ...
      let H = e.method || 'GET',
        ...
        F = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, a) => P.onRequestError(e, t, a, x),
          },
          sharedContext: { buildId: b },
        },
        K = new c.NodeNextRequest(e),
      ...
      try {
          ...
          l = async (i) => {
            ...
            let c = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!O)
                    return (await (0, g.sendResponse)(K, W, n, F.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await n.blob(),
                      t = (0, v.toNodeOutgoingHttpHeaders)(n.headers)
                    ;(c && (t[m.NEXT_CACHE_TAGS_HEADER] = c),
            ...
            if (!O) return null
            if (
              (null == d || null == (n = d.value) ? void 0 : n.kind) !== R.CachedRouteKind.APP_ROUTE
            )
            ...
            return (
              ...
              !d.cacheControl ||
                ...
                u.get('Cache-Control') ||
                u.set('Cache-Control', (0, h.getCacheControlHeader)(d.cacheControl)),
        ...
        if (
          ...
          throw t
        return (await (0, g.sendResponse)(K, W, new Response(null, { status: 500 })), null)
      }
    }
    e.s(
      [
...
//# sourceMappingURL=%5Broot-of-the-server%5D__38f3ce75._.js.map
