module.exports = [
  ...
  (e) => {
    ...
    async function E(e) {
      try {
        let t = await w.default.api.getSession({ headers: await (0, A.headers)() })
        if (!t || t.user?.role !== 'ADMIN')
          return _.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        let { searchParams: r } = new URL(e.url),
          o = parseInt(r.get('page') || '1'),
          a = parseInt(r.get('limit') || '20'),
          n = r.get('search') || '',
          i = r.get('status') || 'ALL',
          s = r.get('location') || 'ALL',
          l = { role: 'GROOMER' }
        ;(n &&
          (l.OR = [
            { name: { contains: n, mode: 'insensitive' } },
            { email: { contains: n, mode: 'insensitive' } },
            { phoneNumber: { contains: n, mode: 'insensitive' } },
          ]),
          'ALL' !== i && (l.groomerProfile = { isActive: 'ACTIVE' === i }),
          'ALL' !== s && (l.workAreas = { some: { id: s } }))
        let [d, c] = await Promise.all([
            b.prisma.user.findMany({
              where: l,
              include: {
                ...
                groomerBookings: {
                  ...
                  select: { id: !0, totalPrice: !0, createdAt: !0 },
                },
            ...
            b.prisma.user.count({ where: l }),
          ]),
          u = d.map((e) => {
            let t = e.reviews.map((e) => e.rating),
              r = t.length > 0 ? t.reduce((e, t) => e + t, 0) / t.length : 0,
              o = new Date()
            o.setDate(o.getDate() - 30)
            let a = e.groomerBookings
                .filter((e) => new Date(e.createdAt) >= o)
                .reduce((e, t) => e + t.totalPrice, 0),
              n =
                e.groomerBookings.length > 0
                  ? e.groomerBookings.sort(
            ...
            return {
              ...
              user: {
                ...
                isActive: e.groomerProfile?.isActive ?? !0,
              ...
              portfolio: [],
              birthDate: e.groomerProfile?.birthDate
                ? e.groomerProfile.birthDate.toISOString()
                : null,
              availableLocations: e.workAreas.map((e) => ({
                id: e.id,
                name: e.name,
                address: e.address || '',
              ...
              services: [],
              bankAccount: e.groomerProfile
                ? {
                    bankName: e.groomerProfile.bankName || '',
                    accountNumber: e.groomerProfile.bankAccountNumber || '',
                    accountHolder: e.groomerProfile.bankAccountHolderName || '',
                  }
                : null,
              commissionGrade: e.groomerProfile?.commissionGrade
                ? {
                    id: e.groomerProfile.commissionGrade.id,
                    name: e.groomerProfile.commissionGrade.name,
                    commissionRate: e.groomerProfile.commissionGrade.commissionRate,
                  }
                : null,
              lastActivityAt: n ? new Date(n).toISOString() : null,
            }
          }),
          m = Math.ceil(c / a)
        return _.NextResponse.json({ groomers: u, totalCount: c, totalPages: m, currentPage: o })
      } catch (e) {
        return (
    ...
    let k = new t.AppRouteRouteModule({
        definition: {
          ...
          page: '/api/admin/groomers/route',
          pathname: '/api/admin/groomers',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/admin/groomers/route.ts',
    ...
    async function T(e, t, o) {
      ...
      let _ = '/api/admin/groomers/route'
      _ = _.replace(/\/index$/, '') || '/'
      ...
      let {
        ...
        j = !!(C.dynamicRoutes[D] || C.routes[O]),
        L = async () => (
      ...
      if (j && !x) {
        let e = !!C.routes[O],
          t = C.dynamicRoutes[D]
        if (t && !1 === t.fallback && !e) {
          if (E.experimental.adapterPath) return await L()
          throw new f.NoFallbackError()
        }
      }
      let M = null
      !j || k.isDev || x || (M = '/index' === (M = O) ? '/' : M)
      let H = !0 === k.isDev || !j,
        U = j && !H
      I &&
        S &&
      ...
      let q = e.method || 'GET',
        ...
        V = c.NextRequestAdapter.fromNodeNextRequest(F, (0, c.signalFromNodeResponse)(t))
      try {
        let i = async (e) =>
            k.handle(V, K).finally(() => {
              if (!e) return
          ...
          l = async (a) => {
            ...
            let d = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!j)
                    return (await (0, p.sendResponse)(F, $, n, K.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await n.blob(),
        ...
        if (
          ...
          throw t
        return (await (0, p.sendResponse)(F, $, new Response(null, { status: 500 })), null)
      }
    }
    e.s(
      [
