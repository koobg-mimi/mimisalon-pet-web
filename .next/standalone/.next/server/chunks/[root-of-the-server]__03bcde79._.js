module.exports = [
  ...
  (e, t, r) => {
    ...
    t.exports = function e() {
      var t,
        ...
        g = !1
      for (
        'boolean' == typeof c && ((g = c), (c = arguments[1] || {}), (p = 2)),
  ...
  (e) => {
    ...
    var R = e.i(600220),
      ...
      E = e.i(820908)
    let j = y.z.object({
      imageData: y.z.string().min(1, 'Image data is required'),
      filename: y.z.string().min(1, 'Filename is required'),
      mimeType: y.z.enum(['image/jpeg', 'image/png', 'image/webp', 'image/gif'], {
    ...
    async function P(e) {
      try {
        let t,
          r = await x.default.api.getSession({ headers: await (0, w.headers)() })
        if (!r || r.user?.role !== 'GROOMER')
          return _.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        if (!(0, E.isGcsConfigured)())
          return _.NextResponse.json(
            { error: 'Image upload service not configured' },
            { status: 503 }
          )
        let a = await e.json(),
          { imageData: n, filename: o, mimeType: i } = j.parse(a)
        try {
          let e = n.replace(/^data:image\/[a-z]+;base64,/, '')
          t = Buffer.from(e, 'base64')
        } catch (e) {
          return (
            console.error('Failed to parse Base64 data:', e),
            _.NextResponse.json({ error: 'Invalid Base64 image data' }, { status: 400 })
          )
        }
        if (t.length > 5242880)
          return _.NextResponse.json(
            { error: `File too large. Maximum size is ${Math.round(5)}MB` },
            { status: 400 }
          )
        let s =
            o.split('.').pop() ||
              ...
              : i.includes('png')
                ...
                : i.includes('webp')
                  ...
                  : i.includes('gif')
                    ? 'gif'
                    : 'jpg'),
          l = (0, E.generateProfileImageFilename)(r.user.id, s)
        ...
        let u = await b.prisma.user.findUnique({ where: { id: r.user.id }, select: { image: !0 } })
        if (u?.image && (0, E.isGcsUrl)(u.image)) {
          let e = (0, E.extractFilenameFromUrl)(u.image)
          e && (await (0, E.deleteImage)(e).catch(console.error))
        }
        let d = await (0, E.uploadBuffer)(t, l, i, {
            userid: r.user.id,
            uploadedby: r.user.email || r.user.id,
          }),
          c = (
        ...
        if (c && (0, E.isGcsUrl)(c)) {
          let e = (0, E.extractFilenameFromUrl)(c)
          if (e)
            try {
              c = await (0, E.generateSignedReadUrl)(e, 60)
            } catch (e) {
              console.error('Failed to generate signed URL for response:', e)
            }
        }
        return _.NextResponse.json({
          success: !0,
        ...
        return (
          console.error('Error uploading image via server proxy:', e),
    ...
    async function C() {
      try {
        let e = await x.default.api.getSession({ headers: await (0, w.headers)() })
        if (!e || e.user?.role !== 'GROOMER')
          return _.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        let t = await b.prisma.user.findUnique({ where: { id: e.user.id }, select: { image: !0 } })
        if (t?.image && (0, E.isGcsUrl)(t.image)) {
          let e = (0, E.extractFilenameFromUrl)(t.image)
          e && (await (0, E.deleteImage)(e).catch(console.error))
        }
        return (
          await b.prisma.user.update({ where: { id: e.user.id }, data: { image: null } }),
          _.NextResponse.json({ success: !0, message: 'Profile image removed successfully' })
        )
      } catch (e) {
        return (
    ...
    e.s(['DELETE', () => C, 'POST', () => P, 'uploadProfileImageSchema', 0, j], 781941)
    var O = e.i(781941)
    let A = new t.AppRouteRouteModule({
        definition: {
          kind: r.RouteKind.APP_ROUTE,
          page: '/api/groomer/profile/image/upload/route',
          pathname: '/api/groomer/profile/image/upload',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/groomer/profile/image/upload/route.ts',
        nextConfigOutput: 'standalone',
        userland: O,
      }),
      { workAsyncStorage: N, workUnitAsyncStorage: U, serverHooks: I } = A
    function T() {
      return (0, a.patchFetch)({ workAsyncStorage: N, workUnitAsyncStorage: U })
    }
    async function S(e, t, a) {
      A.isDev && (0, n.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let _ = '/api/groomer/profile/image/upload/route'
      _ = _.replace(/\/index$/, '') || '/'
      let w = await A.prepare(e, t, { srcPage: _, multiZoneDraftMode: !1 })
      if (!w)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        S = (0, l.normalizeAppPath)(_),
        k = !!(P.dynamicRoutes[S] || P.routes[U]),
        q = async () => (
      ...
      T &&
        ...
        (0, i.setReferenceManifestsSingleton)({
          ...
          clientReferenceManifest: I,
          serverActionsManifest: T,
          serverModuleMap: (0, s.createServerModuleMap)({ serverActionsManifest: T }),
        })
      let D = e.method || 'GET',
        G = (0, o.getTracer)(),
        ...
        B = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, a) => A.onRequestError(e, t, a, C),
          },
          sharedContext: { buildId: y },
        },
        $ = new u.NodeNextRequest(e),
        K = new u.NodeNextResponse(t),
        L = d.NextRequestAdapter.fromNodeNextRequest($, (0, d.signalFromNodeResponse)(t))
      try {
        let i = async (e) =>
            A.handle(L, B).finally(() => {
              if (!e) return
          ...
          l = async (n) => {
            ...
            let u = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!k)
                    return (await (0, m.sendResponse)($, K, o, B.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await o.blob(),
                    ...
                    let r =
                        ...
                        !(B.renderOpts.collectedRevalidate >= h.INFINITE_CACHE) &&
                        B.renderOpts.collectedRevalidate,
                      a =
                        void 0 === B.renderOpts.collectedExpire ||
                        B.renderOpts.collectedExpire >= h.INFINITE_CACHE
                          ? void 0
            ...
            let c = (0, g.fromNodeOutgoingHttpHeaders)(d.value.headers)
            return (
              (s && k) || c.delete(h.NEXT_CACHE_TAGS_HEADER),
              !d.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__03bcde79._.js.map
