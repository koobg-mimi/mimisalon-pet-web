module.exports = [
  ...
  (e, t, r) => {
    ...
    t.exports = function e() {
      var t,
        ...
        m = !1
      for (
        'boolean' == typeof c && ((m = c), (c = arguments[1] || {}), (p = 2)),
        ...
        if (((t = arguments[p]), null != t))
          for (r in t)
            ((a = d(c, r)),
              c !== (n = d(t, r)) &&
                (m && n && (l(n) || (i = o(n)))
                  ? (i ? ((i = !1), (s = a && o(a) ? a : [])) : (s = a && l(a) ? a : {}),
                    u(c, { name: r, newValue: e(m, s, n) }))
  ...
  (e) => {
    ...
    var R = e.i(600220),
      ...
      E = e.i(820908)
    let P = y.z.object({
      isPrimary: y.z.boolean().optional(),
      displayOrder: y.z.number().int().min(0).optional(),
    })
    async function I(e, { params: t }) {
      try {
        let { id: r, imageId: a } = await t,
          n = await x.default.api.getSession({ headers: await (0, _.headers)() })
        if (!n?.user?.id) return new g.NextResponse('Unauthorized', { status: 401 })
        if (
          !(await b.prisma.pet.findFirst({ where: { id: r, customerId: n.user.id, isActive: !0 } }))
        )
          return new g.NextResponse('Pet not found', { status: 404 })
        if (!(await b.prisma.petImage.findFirst({ where: { id: a, petId: r } })))
          return new g.NextResponse('Image not found', { status: 404 })
        let i = await e.json(),
          s = P.parse(i)
        !0 === s.isPrimary &&
          (await b.prisma.petImage.updateMany({
            where: { petId: r, id: { not: a }, isPrimary: !0 },
            data: { isPrimary: !1 },
          }))
        let o = await b.prisma.petImage.update({ where: { id: a }, data: s })
        return g.NextResponse.json(o)
      } catch (e) {
    ...
    async function A(e, { params: t }) {
      try {
        let { id: e, imageId: r } = await t,
          a = await x.default.api.getSession({ headers: await (0, _.headers)() })
        if (!a?.user?.id) return new g.NextResponse('Unauthorized', { status: 401 })
        if (
          !(await b.prisma.pet.findFirst({ where: { id: e, customerId: a.user.id, isActive: !0 } }))
        )
          return new g.NextResponse('Pet not found', { status: 404 })
        let n = await b.prisma.petImage.findFirst({ where: { id: r, petId: e } })
        if (!n) return new g.NextResponse('Image not found', { status: 404 })
        try {
          let e = n.url.split('/').slice(-3).join('/')
          await (0, E.deleteImage)(e)
        } catch (e) {
          console.error(`Failed to delete image from GCS: ${n.url}`, e)
        }
        if ((await b.prisma.petImage.delete({ where: { id: r } }), n.isPrimary)) {
          let t = await b.prisma.petImage.findFirst({
            where: { petId: e },
            orderBy: { createdAt: 'asc' },
          })
          t && (await b.prisma.petImage.update({ where: { id: t.id }, data: { isPrimary: !0 } }))
        }
        return g.NextResponse.json({ message: 'Image deleted successfully' })
      } catch (e) {
        return (
    ...
    e.s(['DELETE', () => A, 'PUT', () => I, 'updateImageSchema', 0, P], 309012)
    var C = e.i(309012)
    let N = new t.AppRouteRouteModule({
        definition: {
          kind: r.RouteKind.APP_ROUTE,
          page: '/api/customer/pets/[id]/images/[imageId]/route',
          pathname: '/api/customer/pets/[id]/images/[imageId]',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/customer/pets/[id]/images/[imageId]/route.ts',
        nextConfigOutput: 'standalone',
        userland: C,
      }),
      { workAsyncStorage: j, workUnitAsyncStorage: O, serverHooks: S } = N
    function T() {
    ...
    async function k(e, t, a) {
      ...
      let g = '/api/customer/pets/[id]/images/[imageId]/route'
      g = g.replace(/\/index$/, '') || '/'
      ...
      let {
        ...
        k = (0, l.normalizeAppPath)(g),
        q = !!(I.dynamicRoutes[k] || I.routes[O]),
        U = async () => (
      ...
      T &&
        ...
        (0, s.setReferenceManifestsSingleton)({
          ...
          clientReferenceManifest: S,
          serverActionsManifest: T,
          serverModuleMap: (0, o.createServerModuleMap)({ serverActionsManifest: T }),
        })
      let D = e.method || 'GET',
        $ = (0, i.getTracer)(),
        ...
        z = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, a) => N.onRequestError(e, t, a, A),
          },
          sharedContext: { buildId: y },
        },
        V = new u.NodeNextRequest(e),
        B = new u.NodeNextResponse(t),
        L = d.NextRequestAdapter.fromNodeNextRequest(V, (0, d.signalFromNodeResponse)(t))
      try {
        let s = async (e) =>
            N.handle(L, z).finally(() => {
              if (!e) return
          ...
          l = async (n) => {
            ...
            let u = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!q)
                    return (await (0, h.sendResponse)(V, B, i, z.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await i.blob(),
                    ...
                    let r =
                        ...
                        !(z.renderOpts.collectedRevalidate >= w.INFINITE_CACHE) &&
                        z.renderOpts.collectedRevalidate,
                      a =
                        void 0 === z.renderOpts.collectedExpire ||
                        z.renderOpts.collectedExpire >= w.INFINITE_CACHE
                          ? void 0
            ...
            return (
              (o && q) || c.delete(w.NEXT_CACHE_TAGS_HEADER),
              !d.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__ff43e51b._.js.map
