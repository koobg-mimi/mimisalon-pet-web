module.exports = [
  ...
  (e) => {
    ...
    var _ = e.i(600220),
      ...
      b = e.i(469719)
    let x = b.z.object({
      name: b.z.string().min(1).optional(),
      phoneNumber: b.z.string().optional(),
    })
    async function N() {
      try {
        ...
        let t = await E.prisma.user.findUnique({
          where: { id: e.user.id },
          select: {
            id: !0,
            name: !0,
        ...
        if (!t) return g.NextResponse.json({ error: 'Profile not found' }, { status: 404 })
        let r = {
          id: t.id,
          name: t.name || '',
          email: t.email,
    ...
    async function C(e) {
      try {
        ...
        if (!t || t.user?.role !== 'ADMIN')
          return g.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        let r = await e.json(),
          { name: a, phoneNumber: n } = x.parse(r),
          o = await E.prisma.user.update({
            where: { id: t.user.id },
            data: { name: a || void 0, phoneNumber: n || void 0 },
            select: {
              id: !0,
              name: !0,
              email: !0,
    ...
    let P = new t.AppRouteRouteModule({
      ...
      { workAsyncStorage: j, workUnitAsyncStorage: T, serverHooks: k } = P
    function O() {
      return (0, a.patchFetch)({ workAsyncStorage: j, workUnitAsyncStorage: T })
    }
    async function S(e, t, a) {
      P.isDev && (0, n.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let g = '/api/admin/profile/route'
      g = g.replace(/\/index$/, '') || '/'
      let A = await P.prepare(e, t, { srcPage: g, multiZoneDraftMode: !1 })
      if (!A)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
          ...
          l = async (n) => {
            ...
            let d = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!U)
                    return (await (0, h.sendResponse)(L, z, o, B.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await o.blob(),
            ...
            let p = (0, m.fromNodeOutgoingHttpHeaders)(u.value.headers)
            return (
              (i && U) || p.delete(f.NEXT_CACHE_TAGS_HEADER),
              !u.cacheControl ||
                t.getHeader('Cache-Control') ||
                p.get('Cache-Control') ||
                p.set('Cache-Control', (0, v.getCacheControlHeader)(u.cacheControl)),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__3b436c19._.js.map
