module.exports = [
  ...
  (e) => {
    ...
    var _ = e.i(600220),
      ...
      x = e.i(469719)
    let y = x.z.object({
      name: x.z.string().min(1, '옵션 이름을 입력해주세요'),
      description: x.z.string().optional(),
      price: x.z.number().positive('가격은 0보다 커야 합니다'),
      applicableCategories: x.z
        .array(x.z.nativeEnum(b.BreedCategory))
        .min(1, '최소 1개 이상의 품종 카테고리를 선택해주세요'),
      displayOrder: x.z.number().int().default(0),
      isActive: x.z.boolean().default(!0),
    })
    async function C(e) {
      try {
        let t = await w.default.api.getSession({ headers: await (0, E.headers)() })
        if (!t?.user || 'ADMIN' !== t.user.role)
          return g.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        let r = e.nextUrl.searchParams,
          a = r.get('isActive'),
          n = r.get('breedCategory'),
          s = {}
        ;(null !== a && (s.isActive = 'true' === a), n && (s.applicableCategories = { has: n }))
        let o = await b.prisma.serviceOption.findMany({
          where: s,
        ...
        return (
          console.error('Error fetching service options:', e),
    ...
    async function A(e) {
      try {
        ...
        let r = await e.json(),
          a = y.parse(r),
          n = await b.prisma.serviceOption.create({ data: a })
        return g.NextResponse.json(n, { status: 201 })
      } catch (e) {
        if (e instanceof x.z.ZodError)
          return g.NextResponse.json(
            { error: '입력 데이터가 올바르지 않습니다', details: e.issues },
            { status: 400 }
          )
        return (
          console.error('Error creating service option:', e),
          g.NextResponse.json({ error: 'Internal Server Error' }, { status: 500 })
        )
      }
    }
    e.s(['GET', () => C, 'POST', () => A], 67333)
    var N = e.i(67333)
    let P = new t.AppRouteRouteModule({
        definition: {
          kind: r.RouteKind.APP_ROUTE,
          page: '/api/admin/service-options/route',
          pathname: '/api/admin/service-options',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/admin/service-options/route.ts',
    ...
    async function S(e, t, a) {
      ...
      let g = '/api/admin/service-options/route'
      g = g.replace(/\/index$/, '') || '/'
      let E = await P.prepare(e, t, { srcPage: g, multiZoneDraftMode: !1 })
      if (!E)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        M = !!(A.dynamicRoutes[I] || A.routes[T]),
        U = async () => (
...
//# sourceMappingURL=%5Broot-of-the-server%5D__65783f8c._.js.map
