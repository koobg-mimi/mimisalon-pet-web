module.exports = [
        ...
        if (((t = arguments[p]), null != t))
          for (r in t)
            ((a = c(d, r)),
              d !== (n = c(t, r)) &&
                (m && n && (l(n) || (o = s(n)))
                  ? (o ? ((o = !1), (i = a && s(a) ? a : [])) : (i = a && l(a) ? a : {}),
                    u(d, { name: r, newValue: e(m, i, n) }))
                  : void 0 !== n && u(d, { name: r, newValue: n })))
      return d
    }
  },
  624479,
  (e) => {
    ...
    var _ = e.i(600220),
      ...
      x = e.i(820908)
    let A = w.z.object({
      name: w.z.string().min(1, 'Pet name is required'),
      type: w.z.enum(['DOG', 'CAT']),
      breedId: w.z.string().optional(),
      weight: w.z.number().positive().optional(),
      age: w.z.number().int().positive().optional(),
      birthDate: w.z.string().datetime().optional(),
      gender: w.z.enum(['MALE', 'FEMALE', 'UNKNOWN']).optional(),
      hairType: w.z.enum(['SHORT_HAIR', 'LONG_HAIR']).optional().nullable(),
      specialNeeds: w.z.string().optional(),
      vaccinationStatus: w.z.enum(['UP_TO_DATE', 'OVERDUE', 'PARTIAL', 'UNKNOWN']).optional(),
      vaccinationDate: w.z.string().datetime().optional(),
    })
    async function P() {
      try {
        let e = await b.default.api.getSession({ headers: await (0, y.headers)() })
        if (!e?.user?.id) return new Response('Unauthorized', { status: 401 })
        let t = await E.prisma.pet.findMany({
            where: { customerId: e.user.id, isActive: !0 },
            include: { breed: !0, images: { where: { isPrimary: !0 }, take: 1 } },
          ...
          r = await Promise.all(
            t.map(async (e) => {
              ...
              let t = await Promise.all(
                e.images.map(async (e) => {
                  if ('string' != typeof e.url)
                    return (console.warn(`Invalid image URL type: ${typeof e.url}`), e)
                  if (!(0, x.isGcsUrl)(e.url)) return e
                  let t = (0, x.extractFilenameFromUrl)(e.url)
                  if (!t)
                    return (console.warn(`Could not extract filename from GCS URL: ${e.url}`), e)
                  try {
                    let r = await (0, x.generateSignedReadUrl)(t, 60)
                    return { ...e, url: r }
                  } catch (r) {
                    return (console.error(`Failed to generate signed URL for ${t}:`, r), e)
                  }
                })
              )
              return { ...e, images: t }
          ...
          a = g.NextResponse.json(r)
        return (
          a.headers.set('Cache-Control', 'no-cache, no-store, must-revalidate'),
    ...
    async function C(e) {
      try {
        let t = await b.default.api.getSession({ headers: await (0, y.headers)() })
        if (!t?.user?.id) return new g.NextResponse('Unauthorized', { status: 401 })
        let r = await e.json(),
          a = A.parse(r)
        'DOG' === a.type && a.hairType && (a.hairType = null)
        let n = await E.prisma.pet.create({
          data: {
            ...a,
            birthDate: a.birthDate ? new Date(a.birthDate) : void 0,
            vaccinationDate: a.vaccinationDate ? new Date(a.vaccinationDate) : void 0,
            customerId: t.user.id,
          },
          include: { breed: !0, images: !0 },
        })
        return g.NextResponse.json(n)
      } catch (e) {
        if (e instanceof w.z.ZodError)
    ...
    e.s(['GET', () => P, 'POST', () => C, 'createPetSchema', 0, A], 500356)
    var O = e.i(500356)
    let N = new t.AppRouteRouteModule({
        definition: {
          kind: r.RouteKind.APP_ROUTE,
          page: '/api/customer/pets/route',
          pathname: '/api/customer/pets',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/customer/pets/route.ts',
        nextConfigOutput: 'standalone',
        userland: O,
      }),
      { workAsyncStorage: T, workUnitAsyncStorage: j, serverHooks: S } = N
    function D() {
      return (0, a.patchFetch)({ workAsyncStorage: T, workUnitAsyncStorage: j })
    }
    async function U(e, t, a) {
      N.isDev && (0, n.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let g = '/api/customer/pets/route'
      g = g.replace(/\/index$/, '') || '/'
      let y = await N.prepare(e, t, { srcPage: g, multiZoneDraftMode: !1 })
      if (!y)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        k = !!(P.dynamicRoutes[U] || P.routes[j]),
        I = async () => (
      ...
      if (k && !A) {
        let e = !!P.routes[j],
          t = P.dynamicRoutes[U]
        if (t && !1 === t.fallback && !e) {
          if (E.experimental.adapterPath) return await I()
          throw new R.NoFallbackError()
        }
      }
      let q = null
      !k || N.isDev || A || (q = '/index' === (q = j) ? '/' : q)
      let H = !0 === N.isDev || !k,
        M = k && !H
      D &&
        S &&
      ...
      let z = e.method || 'GET',
        ...
        $ = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, a) => N.onRequestError(e, t, a, C),
          },
      ...
      try {
          ...
          l = async (n) => {
            ...
            let u = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!k)
                    return (await (0, h.sendResponse)(K, G, o, $.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await o.blob(),
                    ...
                    let r =
                        ...
                        !($.renderOpts.collectedRevalidate >= v.INFINITE_CACHE) &&
                        $.renderOpts.collectedRevalidate,
                      a =
                        void 0 === $.renderOpts.collectedExpire ||
                        $.renderOpts.collectedExpire >= v.INFINITE_CACHE
                          ? void 0
            ...
            if (!k) return null
            if (
              (null == c || null == (o = c.value) ? void 0 : o.kind) !== _.CachedRouteKind.APP_ROUTE
            )
            ...
            return (
              (s && k) || d.delete(v.NEXT_CACHE_TAGS_HEADER),
              !c.cacheControl ||
        ...
        if (
          ...
          throw t
        return (await (0, h.sendResponse)(K, G, new Response(null, { status: 500 })), null)
      }
    }
    e.s(
      [
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c86080d2._.js.map
