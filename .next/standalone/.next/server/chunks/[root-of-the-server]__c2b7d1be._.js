  ...
  (e) => {
    ...
    async function b(e, { params: t }) {
      try {
        ...
        let { id: r } = await t,
          a = await x.prisma.user.findUnique({ where: { email: e.user.email } })
        if (!a) return w.NextResponse.json({ error: 'User not found' }, { status: 404 })
        let n = await x.prisma.review.findUnique({ where: { id: r, customerId: a.id } })
        if (!n)
          return w.NextResponse.json({ error: 'Review not found or unauthorized' }, { status: 404 })
        return (
          await x.prisma.review.delete({ where: { id: r } }),
          await x.prisma.booking.update({
            where: { id: n.bookingId },
            data: { customerRating: null, customerReview: null, reviewDate: null },
          }),
          w.NextResponse.json({ success: !0 })
        )
      } catch (e) {
        return (
          console.error('Review deletion error:', e),
          w.NextResponse.json({ error: 'Failed to delete review' }, { status: 500 })
        )
      }
    }
    e.s(['DELETE', () => b], 980455)
    ...
    let y = new t.AppRouteRouteModule({
        definition: {
          ...
          page: '/api/customer/reviews/[id]/route',
          pathname: '/api/customer/reviews/[id]',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/customer/reviews/[id]/route.ts',
    ...
    async function j(e, t, a) {
      ...
      let w = '/api/customer/reviews/[id]/route'
      w = w.replace(/\/index$/, '') || '/'
      ...
      let {
        ...
        q = !!(N.dynamicRoutes[U] || N.routes[T]),
        H = async () => (
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c2b7d1be._.js.map
