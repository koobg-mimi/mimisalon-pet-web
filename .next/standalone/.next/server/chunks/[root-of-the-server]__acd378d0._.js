module.exports = [
  ...
  (e) => {
    ...
    async function C(e, { params: t }) {
      try {
        ...
        let { id: r, action: a } = await t,
          s = await E.prisma.groomerCommissionGrade.findUnique({
            where: { id: r },
        ...
        switch (a) {
          case 'activate':
            if (s.isActive)
              return g.NextResponse.json({ error: '이미 활성화된 등급입니다' }, { status: 400 })
            ;((n = { isActive: !0 }), (o = '커미션 등급이 활성화되었습니다'))
            break
          case 'deactivate':
            if (!s.isActive)
              return g.NextResponse.json({ error: '이미 비활성화된 등급입니다' }, { status: 400 })
            if (s._count.groomers > 0)
              return g.NextResponse.json(
                { error: '미용사가 사용 중인 등급은 비활성화할 수 없습니다' },
                { status: 400 }
              )
            ;((n = { isActive: !1 }), (o = '커미션 등급이 비활성화되었습니다'))
            break
          case 'delete':
            if (s._count.groomers > 0)
              return g.NextResponse.json(
            ...
            return (
              await E.prisma.groomerCommissionGrade.delete({ where: { id: r } }),
              g.NextResponse.json({ success: !0, message: '커미션 등급이 삭제되었습니다' })
            )
          default:
            return g.NextResponse.json({ error: '알 수 없는 액션입니다' }, { status: 400 })
        }
        if ('activate' === a || 'deactivate' === a) {
          let e = await E.prisma.groomerCommissionGrade.update({
            where: { id: r },
            data: n,
            include: { _count: { select: { groomers: !0 } } },
          })
        ...
        return g.NextResponse.json({ error: 'Unknown error' }, { status: 500 })
      } catch (e) {
        return (
    ...
    async function A(e, { params: t }) {
      return 'delete' === (await t).action
        ? C(e, { params: t })
        : g.NextResponse.json({ error: '지원되지 않는 액션입니다' }, { status: 400 })
    }
    e.s(['DELETE', () => A, 'PATCH', () => C], 227046)
    var b = e.i(227046)
    let N = new t.AppRouteRouteModule({
        definition: {
          kind: r.RouteKind.APP_ROUTE,
          page: '/api/admin/commission-grades/[id]/[action]/route',
          pathname: '/api/admin/commission-grades/[id]/[action]',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/admin/commission-grades/[id]/[action]/route.ts',
        nextConfigOutput: 'standalone',
        userland: b,
      }),
      { workAsyncStorage: j, workUnitAsyncStorage: y, serverHooks: P } = N
    function k() {
      return (0, a.patchFetch)({ workAsyncStorage: j, workUnitAsyncStorage: y })
    }
    async function T(e, t, a) {
      N.isDev && (0, s.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let g = '/api/admin/commission-grades/[id]/[action]/route'
      g = g.replace(/\/index$/, '') || '/'
      ...
      let {
        ...
        H = !!(j.dynamicRoutes[q] || j.routes[T]),
        U = async () => (
          (null == y ? void 0 : y.render404)
            ? await y.render404(e, t, A, !1)
      ...
      let $ = e.method || 'GET',
        ...
        G = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, a) => N.onRequestError(e, t, a, y),
          },
          ...
          l = async (s) => {
            ...
            let u = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!H)
                    return (await (0, h.sendResponse)(L, B, n, G.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await n.blob(),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__acd378d0._.js.map
