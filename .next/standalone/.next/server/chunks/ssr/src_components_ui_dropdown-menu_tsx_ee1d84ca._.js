module.exports = [
  335732,
  (a) => {
    'use strict'
    var b = a.i(187924),
      c = a.i(572131),
      d = a.i(507554),
      e = a.i(470121),
      f = a.i(750104),
      g = a.i(225152),
      h = a.i(30553),
      i = a.i(37738),
      j = a.i(507827),
      k = a.i(596743),
      l = a.i(486228),
      m = a.i(422297),
      n = a.i(392843),
      o = a.i(704691),
      p = a.i(692616),
      q = a.i(777192),
      r = a.i(224050),
      s = a.i(811011),
      t = a.i(746872),
      u = a.i(241852),
      v = a.i(152081),
      w = ['Enter', ' '],
      x = ['ArrowUp', 'PageDown', 'End'],
      y = ['ArrowDown', 'PageUp', 'Home', ...x],
      z = { ltr: [...w, 'ArrowRight'], rtl: [...w, 'ArrowLeft'] },
      A = { ltr: ['ArrowLeft'], rtl: ['ArrowRight'] },
      B = 'Menu',
      [C, D, E] = (0, i.createCollection)(B),
      [F, G] = (0, f.createContextScope)(B, [
        E,
        o.createPopperScope,
        r.createRovingFocusGroupScope,
      ]),
      H = (0, o.createPopperScope)(),
      I = (0, r.createRovingFocusGroupScope)(),
      [J, K] = F(B),
      [L, M] = F(B),
      N = (a) => {
        let {
            __scopeMenu: d,
            open: e = !1,
            children: f,
            dir: g,
            onOpenChange: h,
            modal: i = !0,
          } = a,
          k = H(d),
          [l, m] = c.useState(null),
          n = c.useRef(!1),
          p = (0, t.useCallbackRef)(h),
          q = (0, j.useDirection)(g)
        return (
          c.useEffect(() => {
            let a = () => {
                ;((n.current = !0),
                  document.addEventListener('pointerdown', b, { capture: !0, once: !0 }),
                  document.addEventListener('pointermove', b, { capture: !0, once: !0 }))
              },
              b = () => (n.current = !1)
            return (
              document.addEventListener('keydown', a, { capture: !0 }),
              () => {
                ;(document.removeEventListener('keydown', a, { capture: !0 }),
                  document.removeEventListener('pointerdown', b, { capture: !0 }),
                  document.removeEventListener('pointermove', b, { capture: !0 }))
              }
            )
          }, []),
          (0, b.jsx)(o.Root, {
            ...k,
            children: (0, b.jsx)(J, {
              scope: d,
              open: e,
              onOpenChange: p,
              content: l,
              onContentChange: m,
              children: (0, b.jsx)(L, {
                scope: d,
                onClose: c.useCallback(() => p(!1), [p]),
                isUsingKeyboardRef: n,
                dir: q,
                modal: i,
                children: f,
              }),
            }),
          })
        )
      }
    N.displayName = B
    var O = c.forwardRef((a, c) => {
      let { __scopeMenu: d, ...e } = a,
        f = H(d)
      return (0, b.jsx)(o.Anchor, { ...f, ...e, ref: c })
    })
    O.displayName = 'MenuAnchor'
    var P = 'MenuPortal',
      [Q, R] = F(P, { forceMount: void 0 }),
      S = (a) => {
        let { __scopeMenu: c, forceMount: d, children: e, container: f } = a,
          g = K(P, c)
        return (0, b.jsx)(Q, {
          scope: c,
          forceMount: d,
          children: (0, b.jsx)(q.Presence, {
            present: d || g.open,
            children: (0, b.jsx)(p.Portal, { asChild: !0, container: f, children: e }),
          }),
        })
      }
    S.displayName = P
    var T = 'MenuContent',
      [U, V] = F(T),
      W = c.forwardRef((a, c) => {
        let d = R(T, a.__scopeMenu),
          { forceMount: e = d.forceMount, ...f } = a,
          g = K(T, a.__scopeMenu),
          h = M(T, a.__scopeMenu)
        return (0, b.jsx)(C.Provider, {
          scope: a.__scopeMenu,
          children: (0, b.jsx)(q.Presence, {
            present: e || g.open,
            children: (0, b.jsx)(C.Slot, {
              scope: a.__scopeMenu,
              children: h.modal ? (0, b.jsx)(X, { ...f, ref: c }) : (0, b.jsx)(Y, { ...f, ref: c }),
            }),
          }),
        })
      }),
      X = c.forwardRef((a, f) => {
        let g = K(T, a.__scopeMenu),
          h = c.useRef(null),
          i = (0, e.useComposedRefs)(f, h)
        return (
          c.useEffect(() => {
            let a = h.current
            if (a) return (0, u.hideOthers)(a)
          }, []),
          (0, b.jsx)($, {
            ...a,
            ref: i,
            trapFocus: g.open,
            disableOutsidePointerEvents: g.open,
            disableOutsideScroll: !0,
            onFocusOutside: (0, d.composeEventHandlers)(
              a.onFocusOutside,
              (a) => a.preventDefault(),
              { checkForDefaultPrevented: !1 }
            ),
            onDismiss: () => g.onOpenChange(!1),
          })
        )
      }),
      Y = c.forwardRef((a, c) => {
        let d = K(T, a.__scopeMenu)
        return (0, b.jsx)($, {
          ...a,
          ref: c,
          trapFocus: !1,
          disableOutsidePointerEvents: !1,
          disableOutsideScroll: !1,
          onDismiss: () => d.onOpenChange(!1),
        })
      }),
      Z = (0, s.createSlot)('MenuContent.ScrollLock'),
      $ = c.forwardRef((a, f) => {
        let {
            __scopeMenu: g,
            loop: h = !1,
            trapFocus: i,
            onOpenAutoFocus: j,
            onCloseAutoFocus: n,
            disableOutsidePointerEvents: p,
            onEntryFocus: q,
            onEscapeKeyDown: s,
            onPointerDownOutside: t,
            onFocusOutside: u,
            onInteractOutside: w,
            onDismiss: z,
            disableOutsideScroll: A,
            ...B
          } = a,
          C = K(T, g),
          E = M(T, g),
          F = H(g),
          G = I(g),
          J = D(g),
          [L, N] = c.useState(null),
          O = c.useRef(null),
          P = (0, e.useComposedRefs)(f, O, C.onContentChange),
          Q = c.useRef(0),
          R = c.useRef(''),
          S = c.useRef(0),
          V = c.useRef(null),
          W = c.useRef('right'),
          X = c.useRef(0),
          Y = A ? v.RemoveScroll : c.Fragment
        ;(c.useEffect(() => () => window.clearTimeout(Q.current), []), (0, l.useFocusGuards)())
        let $ = c.useCallback((a) => {
          var b, c
          return (
            W.current === V.current?.side &&
            ((b = a),
            !!(c = V.current?.area) &&
              (function (a, b) {
                let { x: c, y: d } = a,
                  e = !1
                for (let a = 0, f = b.length - 1; a < b.length; f = a++) {
                  let g = b[a],
                    h = b[f],
                    i = g.x,
                    j = g.y,
                    k = h.x,
                    l = h.y
                  j > d != l > d && c < ((k - i) * (d - j)) / (l - j) + i && (e = !e)
                }
                return e
              })({ x: b.clientX, y: b.clientY }, c))
          )
        }, [])
        return (0, b.jsx)(U, {
          scope: g,
          searchRef: R,
          onItemEnter: c.useCallback(
            (a) => {
              $(a) && a.preventDefault()
            },
            [$]
          ),
          onItemLeave: c.useCallback(
            (a) => {
              $(a) || (O.current?.focus(), N(null))
            },
            [$]
          ),
          onTriggerLeave: c.useCallback(
            (a) => {
              $(a) && a.preventDefault()
            },
            [$]
          ),
          pointerGraceTimerRef: S,
          onPointerGraceIntentChange: c.useCallback((a) => {
            V.current = a
          }, []),
          children: (0, b.jsx)(Y, {
            ...(A ? { as: Z, allowPinchZoom: !0 } : void 0),
            children: (0, b.jsx)(m.FocusScope, {
              asChild: !0,
              trapped: i,
              onMountAutoFocus: (0, d.composeEventHandlers)(j, (a) => {
                ;(a.preventDefault(), O.current?.focus({ preventScroll: !0 }))
              }),
              onUnmountAutoFocus: n,
              children: (0, b.jsx)(k.DismissableLayer, {
                asChild: !0,
                disableOutsidePointerEvents: p,
                onEscapeKeyDown: s,
                onPointerDownOutside: t,
                onFocusOutside: u,
                onInteractOutside: w,
                onDismiss: z,
                children: (0, b.jsx)(r.Root, {
                  asChild: !0,
                  ...G,
                  dir: E.dir,
                  orientation: 'vertical',
                  loop: h,
                  currentTabStopId: L,
                  onCurrentTabStopIdChange: N,
                  onEntryFocus: (0, d.composeEventHandlers)(q, (a) => {
                    E.isUsingKeyboardRef.current || a.preventDefault()
                  }),
                  preventScrollOnEntryFocus: !0,
                  children: (0, b.jsx)(o.Content, {
                    role: 'menu',
                    'aria-orientation': 'vertical',
                    'data-state': ay(C.open),
                    'data-radix-menu-content': '',
                    dir: E.dir,
                    ...F,
                    ...B,
                    ref: P,
                    style: { outline: 'none', ...B.style },
                    onKeyDown: (0, d.composeEventHandlers)(B.onKeyDown, (a) => {
                      let b = a.target.closest('[data-radix-menu-content]') === a.currentTarget,
                        c = a.ctrlKey || a.altKey || a.metaKey,
                        d = 1 === a.key.length
                      if (b) {
                        var e
                        let b, f, g, h, i, j
                        ;('Tab' === a.key && a.preventDefault(),
                          !c &&
                            d &&
                            ((e = a.key),
                            (b = R.current + e),
                            (f = J().filter((a) => !a.disabled)),
                            (g = document.activeElement),
                            (h = f.find((a) => a.ref.current === g)?.textValue),
                            (i = (function (a, b, c) {
                              var d
                              let e =
                                  b.length > 1 && Array.from(b).every((a) => a === b[0]) ? b[0] : b,
                                f = c ? a.indexOf(c) : -1,
                                g = ((d = Math.max(f, 0)), a.map((b, c) => a[(d + c) % a.length]))
                              1 === e.length && (g = g.filter((a) => a !== c))
                              let h = g.find((a) => a.toLowerCase().startsWith(e.toLowerCase()))
                              return h !== c ? h : void 0
                            })(
                              f.map((a) => a.textValue),
                              b,
                              h
                            )),
                            (j = f.find((a) => a.textValue === i)?.ref.current),
                            (function a(b) {
                              ;((R.current = b),
                                window.clearTimeout(Q.current),
                                '' !== b && (Q.current = window.setTimeout(() => a(''), 1e3)))
                            })(b),
                            j && setTimeout(() => j.focus())))
                      }
                      let f = O.current
                      if (a.target !== f || !y.includes(a.key)) return
                      a.preventDefault()
                      let g = J()
                        .filter((a) => !a.disabled)
                        .map((a) => a.ref.current)
                      ;(x.includes(a.key) && g.reverse(),
                        (function (a) {
                          let b = document.activeElement
                          for (let c of a)
                            if (c === b || (c.focus(), document.activeElement !== b)) return
                        })(g))
                    }),
                    onBlur: (0, d.composeEventHandlers)(a.onBlur, (a) => {
                      a.currentTarget.contains(a.target) ||
                        (window.clearTimeout(Q.current), (R.current = ''))
                    }),
                    onPointerMove: (0, d.composeEventHandlers)(
                      a.onPointerMove,
                      aB((a) => {
                        let b = a.target,
                          c = X.current !== a.clientX
                        a.currentTarget.contains(b) &&
                          c &&
                          ((W.current = a.clientX > X.current ? 'right' : 'left'),
                          (X.current = a.clientX))
                      })
                    ),
                  }),
                }),
              }),
            }),
          }),
        })
      })
    W.displayName = T
    var _ = c.forwardRef((a, c) => {
      let { __scopeMenu: d, ...e } = a
      return (0, b.jsx)(h.Primitive.div, { role: 'group', ...e, ref: c })
    })
    _.displayName = 'MenuGroup'
    var aa = c.forwardRef((a, c) => {
      let { __scopeMenu: d, ...e } = a
      return (0, b.jsx)(h.Primitive.div, { ...e, ref: c })
    })
    aa.displayName = 'MenuLabel'
    var ab = 'MenuItem',
      ac = 'menu.itemSelect',
      ad = c.forwardRef((a, f) => {
        let { disabled: g = !1, onSelect: i, ...j } = a,
          k = c.useRef(null),
          l = M(ab, a.__scopeMenu),
          m = V(ab, a.__scopeMenu),
          n = (0, e.useComposedRefs)(f, k),
          o = c.useRef(!1)
        return (0, b.jsx)(ae, {
          ...j,
          ref: n,
          disabled: g,
          onClick: (0, d.composeEventHandlers)(a.onClick, () => {
            let a = k.current
            if (!g && a) {
              let b = new CustomEvent(ac, { bubbles: !0, cancelable: !0 })
              ;(a.addEventListener(ac, (a) => i?.(a), { once: !0 }),
                (0, h.dispatchDiscreteCustomEvent)(a, b),
                b.defaultPrevented ? (o.current = !1) : l.onClose())
            }
          }),
          onPointerDown: (b) => {
            ;(a.onPointerDown?.(b), (o.current = !0))
          },
          onPointerUp: (0, d.composeEventHandlers)(a.onPointerUp, (a) => {
            o.current || a.currentTarget?.click()
          }),
          onKeyDown: (0, d.composeEventHandlers)(a.onKeyDown, (a) => {
            let b = '' !== m.searchRef.current
            g ||
              (b && ' ' === a.key) ||
              (w.includes(a.key) && (a.currentTarget.click(), a.preventDefault()))
          }),
        })
      })
    ad.displayName = ab
    var ae = c.forwardRef((a, f) => {
        let { __scopeMenu: g, disabled: i = !1, textValue: j, ...k } = a,
          l = V(ab, g),
          m = I(g),
          n = c.useRef(null),
          o = (0, e.useComposedRefs)(f, n),
          [p, q] = c.useState(!1),
          [s, t] = c.useState('')
        return (
          c.useEffect(() => {
            let a = n.current
            a && t((a.textContent ?? '').trim())
          }, [k.children]),
          (0, b.jsx)(C.ItemSlot, {
            scope: g,
            disabled: i,
            textValue: j ?? s,
            children: (0, b.jsx)(r.Item, {
              asChild: !0,
              ...m,
              focusable: !i,
              children: (0, b.jsx)(h.Primitive.div, {
                role: 'menuitem',
                'data-highlighted': p ? '' : void 0,
                'aria-disabled': i || void 0,
                'data-disabled': i ? '' : void 0,
                ...k,
                ref: o,
                onPointerMove: (0, d.composeEventHandlers)(
                  a.onPointerMove,
                  aB((a) => {
                    i
                      ? l.onItemLeave(a)
                      : (l.onItemEnter(a),
                        a.defaultPrevented || a.currentTarget.focus({ preventScroll: !0 }))
                  })
                ),
                onPointerLeave: (0, d.composeEventHandlers)(
                  a.onPointerLeave,
                  aB((a) => l.onItemLeave(a))
                ),
                onFocus: (0, d.composeEventHandlers)(a.onFocus, () => q(!0)),
                onBlur: (0, d.composeEventHandlers)(a.onBlur, () => q(!1)),
              }),
            }),
          })
        )
      }),
      af = c.forwardRef((a, c) => {
        let { checked: e = !1, onCheckedChange: f, ...g } = a
        return (0, b.jsx)(an, {
          scope: a.__scopeMenu,
          checked: e,
          children: (0, b.jsx)(ad, {
            role: 'menuitemcheckbox',
            'aria-checked': az(e) ? 'mixed' : e,
            ...g,
            ref: c,
            'data-state': aA(e),
            onSelect: (0, d.composeEventHandlers)(g.onSelect, () => f?.(!!az(e) || !e), {
              checkForDefaultPrevented: !1,
            }),
          }),
        })
      })
    af.displayName = 'MenuCheckboxItem'
    var ag = 'MenuRadioGroup',
      [ah, ai] = F(ag, { value: void 0, onValueChange: () => {} }),
      aj = c.forwardRef((a, c) => {
        let { value: d, onValueChange: e, ...f } = a,
          g = (0, t.useCallbackRef)(e)
        return (0, b.jsx)(ah, {
          scope: a.__scopeMenu,
          value: d,
          onValueChange: g,
          children: (0, b.jsx)(_, { ...f, ref: c }),
        })
      })
    aj.displayName = ag
    var ak = 'MenuRadioItem',
      al = c.forwardRef((a, c) => {
        let { value: e, ...f } = a,
          g = ai(ak, a.__scopeMenu),
          h = e === g.value
        return (0, b.jsx)(an, {
          scope: a.__scopeMenu,
          checked: h,
          children: (0, b.jsx)(ad, {
            role: 'menuitemradio',
            'aria-checked': h,
            ...f,
            ref: c,
            'data-state': aA(h),
            onSelect: (0, d.composeEventHandlers)(f.onSelect, () => g.onValueChange?.(e), {
              checkForDefaultPrevented: !1,
            }),
          }),
        })
      })
    al.displayName = ak
    var am = 'MenuItemIndicator',
      [an, ao] = F(am, { checked: !1 }),
      ap = c.forwardRef((a, c) => {
        let { __scopeMenu: d, forceMount: e, ...f } = a,
          g = ao(am, d)
        return (0, b.jsx)(q.Presence, {
          present: e || az(g.checked) || !0 === g.checked,
          children: (0, b.jsx)(h.Primitive.span, { ...f, ref: c, 'data-state': aA(g.checked) }),
        })
      })
    ap.displayName = am
    var aq = c.forwardRef((a, c) => {
      let { __scopeMenu: d, ...e } = a
      return (0, b.jsx)(h.Primitive.div, {
        role: 'separator',
        'aria-orientation': 'horizontal',
        ...e,
        ref: c,
      })
    })
    aq.displayName = 'MenuSeparator'
    var ar = c.forwardRef((a, c) => {
      let { __scopeMenu: d, ...e } = a,
        f = H(d)
      return (0, b.jsx)(o.Arrow, { ...f, ...e, ref: c })
    })
    ar.displayName = 'MenuArrow'
    var [as, at] = F('MenuSub'),
      au = 'MenuSubTrigger',
      av = c.forwardRef((a, f) => {
        let g = K(au, a.__scopeMenu),
          h = M(au, a.__scopeMenu),
          i = at(au, a.__scopeMenu),
          j = V(au, a.__scopeMenu),
          k = c.useRef(null),
          { pointerGraceTimerRef: l, onPointerGraceIntentChange: m } = j,
          n = { __scopeMenu: a.__scopeMenu },
          o = c.useCallback(() => {
            ;(k.current && window.clearTimeout(k.current), (k.current = null))
          }, [])
        return (
          c.useEffect(() => o, [o]),
          c.useEffect(() => {
            let a = l.current
            return () => {
              ;(window.clearTimeout(a), m(null))
            }
          }, [l, m]),
          (0, b.jsx)(O, {
            asChild: !0,
            ...n,
            children: (0, b.jsx)(ae, {
              id: i.triggerId,
              'aria-haspopup': 'menu',
              'aria-expanded': g.open,
              'aria-controls': i.contentId,
              'data-state': ay(g.open),
              ...a,
              ref: (0, e.composeRefs)(f, i.onTriggerChange),
              onClick: (b) => {
                ;(a.onClick?.(b),
                  a.disabled ||
                    b.defaultPrevented ||
                    (b.currentTarget.focus(), g.open || g.onOpenChange(!0)))
              },
              onPointerMove: (0, d.composeEventHandlers)(
                a.onPointerMove,
                aB((b) => {
                  ;(j.onItemEnter(b),
                    !b.defaultPrevented &&
                      (a.disabled ||
                        g.open ||
                        k.current ||
                        (j.onPointerGraceIntentChange(null),
                        (k.current = window.setTimeout(() => {
                          ;(g.onOpenChange(!0), o())
                        }, 100)))))
                })
              ),
              onPointerLeave: (0, d.composeEventHandlers)(
                a.onPointerLeave,
                aB((a) => {
                  o()
                  let b = g.content?.getBoundingClientRect()
                  if (b) {
                    let c = g.content?.dataset.side,
                      d = 'right' === c,
                      e = b[d ? 'left' : 'right'],
                      f = b[d ? 'right' : 'left']
                    ;(j.onPointerGraceIntentChange({
                      area: [
                        { x: a.clientX + (d ? -5 : 5), y: a.clientY },
                        { x: e, y: b.top },
                        { x: f, y: b.top },
                        { x: f, y: b.bottom },
                        { x: e, y: b.bottom },
                      ],
                      side: c,
                    }),
                      window.clearTimeout(l.current),
                      (l.current = window.setTimeout(
                        () => j.onPointerGraceIntentChange(null),
                        300
                      )))
                  } else {
                    if ((j.onTriggerLeave(a), a.defaultPrevented)) return
                    j.onPointerGraceIntentChange(null)
                  }
                })
              ),
              onKeyDown: (0, d.composeEventHandlers)(a.onKeyDown, (b) => {
                let c = '' !== j.searchRef.current
                a.disabled ||
                  (c && ' ' === b.key) ||
                  (z[h.dir].includes(b.key) &&
                    (g.onOpenChange(!0), g.content?.focus(), b.preventDefault()))
              }),
            }),
          })
        )
      })
    av.displayName = au
    var aw = 'MenuSubContent',
      ax = c.forwardRef((a, f) => {
        let g = R(T, a.__scopeMenu),
          { forceMount: h = g.forceMount, ...i } = a,
          j = K(T, a.__scopeMenu),
          k = M(T, a.__scopeMenu),
          l = at(aw, a.__scopeMenu),
          m = c.useRef(null),
          n = (0, e.useComposedRefs)(f, m)
        return (0, b.jsx)(C.Provider, {
          scope: a.__scopeMenu,
          children: (0, b.jsx)(q.Presence, {
            present: h || j.open,
            children: (0, b.jsx)(C.Slot, {
              scope: a.__scopeMenu,
              children: (0, b.jsx)($, {
                id: l.contentId,
                'aria-labelledby': l.triggerId,
                ...i,
                ref: n,
                align: 'start',
                side: 'rtl' === k.dir ? 'left' : 'right',
                disableOutsidePointerEvents: !1,
                disableOutsideScroll: !1,
                trapFocus: !1,
                onOpenAutoFocus: (a) => {
                  ;(k.isUsingKeyboardRef.current && m.current?.focus(), a.preventDefault())
                },
                onCloseAutoFocus: (a) => a.preventDefault(),
                onFocusOutside: (0, d.composeEventHandlers)(a.onFocusOutside, (a) => {
                  a.target !== l.trigger && j.onOpenChange(!1)
                }),
                onEscapeKeyDown: (0, d.composeEventHandlers)(a.onEscapeKeyDown, (a) => {
                  ;(k.onClose(), a.preventDefault())
                }),
                onKeyDown: (0, d.composeEventHandlers)(a.onKeyDown, (a) => {
                  let b = a.currentTarget.contains(a.target),
                    c = A[k.dir].includes(a.key)
                  b && c && (j.onOpenChange(!1), l.trigger?.focus(), a.preventDefault())
                }),
              }),
            }),
          }),
        })
      })
    function ay(a) {
      return a ? 'open' : 'closed'
    }
    function az(a) {
      return 'indeterminate' === a
    }
    function aA(a) {
      return az(a) ? 'indeterminate' : a ? 'checked' : 'unchecked'
    }
    function aB(a) {
      return (b) => ('mouse' === b.pointerType ? a(b) : void 0)
    }
    ax.displayName = aw
    var aC = 'DropdownMenu',
      [aD, aE] = (0, f.createContextScope)(aC, [G]),
      aF = G(),
      [aG, aH] = aD(aC),
      aI = (a) => {
        let {
            __scopeDropdownMenu: d,
            children: e,
            dir: f,
            open: h,
            defaultOpen: i,
            onOpenChange: j,
            modal: k = !0,
          } = a,
          l = aF(d),
          m = c.useRef(null),
          [o, p] = (0, g.useControllableState)({
            prop: h,
            defaultProp: i ?? !1,
            onChange: j,
            caller: aC,
          })
        return (0, b.jsx)(aG, {
          scope: d,
          triggerId: (0, n.useId)(),
          triggerRef: m,
          contentId: (0, n.useId)(),
          open: o,
          onOpenChange: p,
          onOpenToggle: c.useCallback(() => p((a) => !a), [p]),
          modal: k,
          children: (0, b.jsx)(N, {
            ...l,
            open: o,
            onOpenChange: p,
            dir: f,
            modal: k,
            children: e,
          }),
        })
      }
    aI.displayName = aC
    var aJ = 'DropdownMenuTrigger',
      aK = c.forwardRef((a, c) => {
        let { __scopeDropdownMenu: f, disabled: g = !1, ...i } = a,
          j = aH(aJ, f),
          k = aF(f)
        return (0, b.jsx)(O, {
          asChild: !0,
          ...k,
          children: (0, b.jsx)(h.Primitive.button, {
            type: 'button',
            id: j.triggerId,
            'aria-haspopup': 'menu',
            'aria-expanded': j.open,
            'aria-controls': j.open ? j.contentId : void 0,
            'data-state': j.open ? 'open' : 'closed',
            'data-disabled': g ? '' : void 0,
            disabled: g,
            ...i,
            ref: (0, e.composeRefs)(c, j.triggerRef),
            onPointerDown: (0, d.composeEventHandlers)(a.onPointerDown, (a) => {
              !g &&
                0 === a.button &&
                !1 === a.ctrlKey &&
                (j.onOpenToggle(), j.open || a.preventDefault())
            }),
            onKeyDown: (0, d.composeEventHandlers)(a.onKeyDown, (a) => {
              !g &&
                (['Enter', ' '].includes(a.key) && j.onOpenToggle(),
                'ArrowDown' === a.key && j.onOpenChange(!0),
                ['Enter', ' ', 'ArrowDown'].includes(a.key) && a.preventDefault())
            }),
          }),
        })
      })
    aK.displayName = aJ
    var aL = (a) => {
      let { __scopeDropdownMenu: c, ...d } = a,
        e = aF(c)
      return (0, b.jsx)(S, { ...e, ...d })
    }
    aL.displayName = 'DropdownMenuPortal'
    var aM = 'DropdownMenuContent',
      aN = c.forwardRef((a, e) => {
        let { __scopeDropdownMenu: f, ...g } = a,
          h = aH(aM, f),
          i = aF(f),
          j = c.useRef(!1)
        return (0, b.jsx)(W, {
          id: h.contentId,
          'aria-labelledby': h.triggerId,
          ...i,
          ...g,
          ref: e,
          onCloseAutoFocus: (0, d.composeEventHandlers)(a.onCloseAutoFocus, (a) => {
            ;(j.current || h.triggerRef.current?.focus(), (j.current = !1), a.preventDefault())
          }),
          onInteractOutside: (0, d.composeEventHandlers)(a.onInteractOutside, (a) => {
            let b = a.detail.originalEvent,
              c = 0 === b.button && !0 === b.ctrlKey,
              d = 2 === b.button || c
            ;(!h.modal || d) && (j.current = !0)
          }),
          style: {
            ...a.style,
            '--radix-dropdown-menu-content-transform-origin':
              'var(--radix-popper-transform-origin)',
            '--radix-dropdown-menu-content-available-width': 'var(--radix-popper-available-width)',
            '--radix-dropdown-menu-content-available-height':
              'var(--radix-popper-available-height)',
            '--radix-dropdown-menu-trigger-width': 'var(--radix-popper-anchor-width)',
            '--radix-dropdown-menu-trigger-height': 'var(--radix-popper-anchor-height)',
          },
        })
      })
    ;((aN.displayName = aM),
      (c.forwardRef((a, c) => {
        let { __scopeDropdownMenu: d, ...e } = a,
          f = aF(d)
        return (0, b.jsx)(_, { ...f, ...e, ref: c })
      }).displayName = 'DropdownMenuGroup'))
    var aO = c.forwardRef((a, c) => {
      let { __scopeDropdownMenu: d, ...e } = a,
        f = aF(d)
      return (0, b.jsx)(aa, { ...f, ...e, ref: c })
    })
    aO.displayName = 'DropdownMenuLabel'
    var aP = c.forwardRef((a, c) => {
      let { __scopeDropdownMenu: d, ...e } = a,
        f = aF(d)
      return (0, b.jsx)(ad, { ...f, ...e, ref: c })
    })
    ;((aP.displayName = 'DropdownMenuItem'),
      (c.forwardRef((a, c) => {
        let { __scopeDropdownMenu: d, ...e } = a,
          f = aF(d)
        return (0, b.jsx)(af, { ...f, ...e, ref: c })
      }).displayName = 'DropdownMenuCheckboxItem'),
      (c.forwardRef((a, c) => {
        let { __scopeDropdownMenu: d, ...e } = a,
          f = aF(d)
        return (0, b.jsx)(aj, { ...f, ...e, ref: c })
      }).displayName = 'DropdownMenuRadioGroup'),
      (c.forwardRef((a, c) => {
        let { __scopeDropdownMenu: d, ...e } = a,
          f = aF(d)
        return (0, b.jsx)(al, { ...f, ...e, ref: c })
      }).displayName = 'DropdownMenuRadioItem'),
      (c.forwardRef((a, c) => {
        let { __scopeDropdownMenu: d, ...e } = a,
          f = aF(d)
        return (0, b.jsx)(ap, { ...f, ...e, ref: c })
      }).displayName = 'DropdownMenuItemIndicator'))
    var aQ = c.forwardRef((a, c) => {
      let { __scopeDropdownMenu: d, ...e } = a,
        f = aF(d)
      return (0, b.jsx)(aq, { ...f, ...e, ref: c })
    })
    ;((aQ.displayName = 'DropdownMenuSeparator'),
      (c.forwardRef((a, c) => {
        let { __scopeDropdownMenu: d, ...e } = a,
          f = aF(d)
        return (0, b.jsx)(ar, { ...f, ...e, ref: c })
      }).displayName = 'DropdownMenuArrow'),
      (c.forwardRef((a, c) => {
        let { __scopeDropdownMenu: d, ...e } = a,
          f = aF(d)
        return (0, b.jsx)(av, { ...f, ...e, ref: c })
      }).displayName = 'DropdownMenuSubTrigger'),
      (c.forwardRef((a, c) => {
        let { __scopeDropdownMenu: d, ...e } = a,
          f = aF(d)
        return (0, b.jsx)(ax, {
          ...f,
          ...e,
          ref: c,
          style: {
            ...a.style,
            '--radix-dropdown-menu-content-transform-origin':
              'var(--radix-popper-transform-origin)',
            '--radix-dropdown-menu-content-available-width': 'var(--radix-popper-available-width)',
            '--radix-dropdown-menu-content-available-height':
              'var(--radix-popper-available-height)',
            '--radix-dropdown-menu-trigger-width': 'var(--radix-popper-anchor-width)',
            '--radix-dropdown-menu-trigger-height': 'var(--radix-popper-anchor-height)',
          },
        })
      }).displayName = 'DropdownMenuSubContent'),
      a.i(606406),
      a.i(352254),
      a.i(171150))
    var aR = a.i(368114)
    function aS({ ...a }) {
      return (0, b.jsx)(aI, { 'data-slot': 'dropdown-menu', ...a })
    }
    function aT({ ...a }) {
      return (0, b.jsx)(aK, { 'data-slot': 'dropdown-menu-trigger', ...a })
    }
    function aU({ className: a, sideOffset: c = 4, ...d }) {
      return (0, b.jsx)(aL, {
        children: (0, b.jsx)(aN, {
          'data-slot': 'dropdown-menu-content',
          sideOffset: c,
          className: (0, aR.cn)(
            'bg-popover text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-50 max-h-(--radix-dropdown-menu-content-available-height) min-w-[8rem] origin-(--radix-dropdown-menu-content-transform-origin) overflow-x-hidden overflow-y-auto rounded-md border p-1 shadow-md',
            a
          ),
          ...d,
        }),
      })
    }
    function aV({ className: a, inset: c, variant: d = 'default', ...e }) {
      return (0, b.jsx)(aP, {
        'data-slot': 'dropdown-menu-item',
        'data-inset': c,
        'data-variant': d,
        className: (0, aR.cn)(
          "focus:bg-accent focus:text-accent-foreground data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-destructive/10 dark:data-[variant=destructive]:focus:bg-destructive/20 data-[variant=destructive]:focus:text-destructive data-[variant=destructive]:*:[svg]:!text-destructive [&_svg:not([class*='text-'])]:text-muted-foreground relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 data-[inset]:pl-8 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
          a
        ),
        ...e,
      })
    }
    function aW({ className: a, inset: c, ...d }) {
      return (0, b.jsx)(aO, {
        'data-slot': 'dropdown-menu-label',
        'data-inset': c,
        className: (0, aR.cn)('px-2 py-1.5 text-sm font-medium data-[inset]:pl-8', a),
        ...d,
      })
    }
    function aX({ className: a, ...c }) {
      return (0, b.jsx)(aQ, {
        'data-slot': 'dropdown-menu-separator',
        className: (0, aR.cn)('bg-border -mx-1 my-1 h-px', a),
        ...c,
      })
    }
    a.s(
      [
        'DropdownMenu',
        () => aS,
        'DropdownMenuContent',
        () => aU,
        'DropdownMenuItem',
        () => aV,
        'DropdownMenuLabel',
        () => aW,
        'DropdownMenuSeparator',
        () => aX,
        'DropdownMenuTrigger',
        () => aT,
      ],
      335732
    )
  },
]

//# sourceMappingURL=src_components_ui_dropdown-menu_tsx_ee1d84ca._.js.map
