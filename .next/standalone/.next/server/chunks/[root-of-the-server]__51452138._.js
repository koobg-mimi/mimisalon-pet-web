module.exports = [
  ...
  (e) => {
    ...
    async function i(e, i, a) {
      let s = await t.prisma.booking.findMany({
        ...
        o = new Set()
      for (let e of s)
        r(e.serviceTime, (e.estimatedDurationMinutes || 60) + 90).forEach((e) => o.add(e))
      let n = a.filter((e) => o.has(e))
    ...
    async function a(e, t, i, a, s) {
      let o = await e.booking.findMany({
          where: {
            groomerId: t,
            serviceDate: i,
            status: { notIn: ['SERVICE_CANCELLED', 'BOOKING_FAILED'] },
        ...
        n = new Set()
      for (let e of o)
        r(e.serviceTime, (e.estimatedDurationMinutes || 60) + 90).forEach((e) => n.add(e))
      let l = a.filter((e) => n.has(e))
      if (l.length > 0) throw Error(`다음 시간은 이미 예약되었습니다: ${l.join(', ')}`)
      let d = await e.groomerSchedule.findUnique({ where: { groomerId: t } }),
        u = d?.id
      for (let r of a)
        await e.groomerAvailability.upsert({
          where: { groomerId_date_timeSlot: { groomerId: t, date: i, timeSlot: r } },
          update: { isBooked: !0, bookingId: s, isAvailable: !1 },
          create: {
            groomerId: t,
            scheduleId: u || '',
            date: i,
  ...
  (e) => {
    ...
    var b = e.i(600220),
      ...
      I = e.i(616300)
    let k = y.z.object({
      idempotencyKey: y.z.string().min(1, '멱등성 키가 필요합니다'),
      petServices: y.z.array(
        y.z.object({
          petId: y.z.string(),
          services: y.z.array(
            y.z.object({
          ...
          options: y.z
            .array(y.z.object({ id: y.z.string(), name: y.z.string(), price: y.z.number() }))
            .default([]),
        })
      ),
      addressId: y.z.string().optional(),
      customerAddress: y.z
        .object({
          street: y.z.string(),
          city: y.z.string(),
          state: y.z.string(),
          zipCode: y.z.string(),
          detailAddress: y.z.string().optional(),
        })
        .optional(),
      groomerId: y.z.string().min(1, '미용사를 선택해주세요'),
      date: y.z.string().min(1, '날짜를 선택해주세요'),
      timeSlot: y.z
        .string()
        .min(1, '시간을 선택해주세요')
        .regex(/^\d{2}:\d{2}$/, '올바른 시간 형식이 아닙니다 (HH:mm)'),
      specialRequests: y.z.string().optional(),
    })
    async function N(e) {
      try {
        let t = await E.default.api.getSession({ headers: await (0, w.headers)() })
        if (!t?.user?.email)
          return R.NextResponse.json({ error: '인증이 필요합니다' }, { status: 401 })
        if ('CUSTOMER' !== t.user.role)
          return R.NextResponse.json({ error: '고객만 예약을 생성할 수 있습니다' }, { status: 403 })
        let r = await e.json(),
          i = k.parse(r),
          a = await _.prisma.user.findUnique({ where: { email: t.user.email } })
        if (!a)
          return R.NextResponse.json({ error: '사용자 정보를 찾을 수 없습니다' }, { status: 404 })
        let s = new Date(Date.now() - 9e5),
          o = await _.prisma.booking.findFirst({
            where: { customerId: a.id, idempotencyKey: i.idempotencyKey, createdAt: { gt: s } },
            include: { bookingPets: { include: { services: { include: { service: !0 } } } } },
          })
        if (o)
          return (
            console.log(
              `[Booking Initialize] Returning existing booking for idempotency key: ${i.idempotencyKey}`
            ),
            R.NextResponse.json({
              bookingId: o.id,
        ...
        let n = await _.prisma.user.findFirst({
          where: { id: i.groomerId, role: 'GROOMER' },
          include: { groomerProfile: !0 },
        })
        if (!n) return R.NextResponse.json({ error: '미용사를 찾을 수 없습니다' }, { status: 404 })
        if (!n.groomerProfile?.isActive)
          return R.NextResponse.json(
            { error: '선택하신 미용사는 현재 예약을 받을 수 없는 상태입니다' },
            { status: 400 }
          )
        let l = i.petServices.reduce(
            (e, t) => e + t.services.reduce((e, t) => e + (t.duration || 60), 0),
            0
          ),
          d = (0, I.generateRequiredTimeSlots)(i.timeSlot, l),
          u = await (0, I.checkGroomerAvailability)(i.groomerId, new Date(i.date), d)
        if (!u.available)
          return R.NextResponse.json(
            {
              error: '선택한 시간대가 이미 예약되었습니다',
              conflicts: u.conflicts,
              message: `다음 시간이 이미 예약되어 있습니다: ${u.conflicts?.join(', ')}`,
            },
            { status: 409 }
          )
        let c = i.petServices.reduce((e, t) => {
            let r = t.services.reduce((e, t) => e + t.price, 0),
              i = t.options.reduce((e, t) => e + t.price, 0)
            return e + r + i
          }, 0),
          p = `BK${Date.now()}`,
          m = await _.prisma.$transaction(async (e) => {
            if (
              await e.booking.findFirst({
                where: {
                  groomerId: i.groomerId,
                  serviceDate: new Date(i.date),
                  status: { notIn: ['SERVICE_CANCELLED', 'BOOKING_FAILED'] },
                  OR: d.map((e) => ({ serviceTime: e })),
                },
              })
            )
              throw Error('시간대가 다른 예약과 충돌합니다. 다른 시간을 선택해주세요.')
            let t = i.addressId
            !t &&
              i.customerAddress &&
              (t = (
                await e.address.create({
                  data: {
                    customerId: a.id,
                    street: i.customerAddress.street,
                    city: i.customerAddress.city,
                    state: i.customerAddress.state,
                    zipCode: i.customerAddress.zipCode,
                    country: 'KR',
                    isDefault: !1,
                  },
                })
              ).id)
            let r = await e.booking.create({
              data: {
                bookingNumber: p,
                customerId: a.id,
                groomerId: i.groomerId,
                ...
                customerAddressId: t,
                estimatedDurationMinutes: l,
                idempotencyKey: i.idempotencyKey,
              },
            })
            for (let t of i.petServices) {
              let i = await e.bookingPet.create({ data: { bookingId: r.id, petId: t.petId } })
              for (let r of t.services)
                await e.bookingService.create({
                  data: { bookingPetId: i.id, serviceId: r.id, servicePrice: r.price },
                })
              for (let r of t.options)
                await e.bookingPetOption.create({
                  data: { bookingPetId: i.id, serviceOptionId: r.id, optionPrice: r.price },
                })
            }
            return (
              await e.booking.update({
                where: { id: r.id },
                data: { expiresAt: new Date(Date.now() + 9e5) },
              }),
              r
            )
          })
        console.log(`[Booking Initialize] Created new pending booking: ${m.id}`)
        let h =
          1 === i.petServices.length
            ? `${i.petServices[0].services[0].name}`
            : `반려동물 ${i.petServices.length}마리 미용서비스`
        return R.NextResponse.json({
          success: !0,
          bookingId: m.id,
          ...
          totalAmount: c,
          orderName: h,
          customerName: a.name || '고객',
          customerEmail: a.email || '',
          customerPhone: a.phoneNumber || '',
          isExisting: !1,
        })
      } catch (e) {
        if (e instanceof y.z.ZodError)
          return R.NextResponse.json(
            { error: '입력 데이터가 올바르지 않습니다', details: e.issues },
            { status: 400 }
          )
        if (e instanceof Error && (e.message.includes('충돌') || e.message.includes('예약')))
          return R.NextResponse.json(
            { error: e.message, code: 'BOOKING_CONFLICT' },
            { status: 409 }
          )
        return (
          console.error('[Booking Initialize] Error:', e),
          R.NextResponse.json({ error: '예약 초기화 중 오류가 발생했습니다' }, { status: 500 })
        )
      }
    }
    e.s(['POST', () => N, 'bookingInitializeSchema', 0, k], 566509)
    var A = e.i(566509)
    let S = new t.AppRouteRouteModule({
        definition: {
          kind: r.RouteKind.APP_ROUTE,
          page: '/api/bookings/initialize/route',
          pathname: '/api/bookings/initialize',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/bookings/initialize/route.ts',
        nextConfigOutput: 'standalone',
        userland: A,
      }),
      { workAsyncStorage: C, workUnitAsyncStorage: x, serverHooks: P } = S
    function O() {
      return (0, i.patchFetch)({ workAsyncStorage: C, workUnitAsyncStorage: x })
    }
    async function T(e, t, i) {
      S.isDev && (0, a.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let R = '/api/bookings/initialize/route'
      R = R.replace(/\/index$/, '') || '/'
      let w = await S.prepare(e, t, { srcPage: R, multiZoneDraftMode: !1 })
      if (!w)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        z = (0, l.normalizeAppPath)(R),
        j = !!(N.dynamicRoutes[z] || N.routes[P]),
        D = async () => (
          (null == A ? void 0 : A.render404)
            ? await A.render404(e, t, I, !1)
      ...
      if (j && !k) {
        let e = !!N.routes[P],
          t = N.dynamicRoutes[z]
        if (t && !1 === t.fallback && !e) {
          if (y.experimental.adapterPath) return await D()
          throw new f.NoFallbackError()
        }
      }
      let M = null
      !j || S.isDev || k || (M = '/index' === (M = P) ? '/' : M)
      let q = !0 === S.isDev || !j,
        U = j && !q
      T &&
        O &&
        (0, o.setReferenceManifestsSingleton)({
          ...
          clientReferenceManifest: O,
          serverActionsManifest: T,
          serverModuleMap: (0, n.createServerModuleMap)({ serverActionsManifest: T }),
        })
      let K = e.method || 'GET',
        $ = (0, s.getTracer)(),
        ...
        F = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, i) => S.onRequestError(e, t, i, A),
          },
      ...
      try {
        let o = async (e) =>
            S.handle(G, F).finally(() => {
              ...
              if (i) {
                let t = `${K} ${i}`
                ;(e.setAttributes({ 'next.route': i, 'http.route': i, 'next.span_name': t }),
                  e.updateName(t))
              } else e.updateName(`${K} ${R}`)
            }),
          ...
          l = async (a) => {
            ...
            let d = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!j)
                    return (await (0, m.sendResponse)(B, L, s, F.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await s.blob(),
            ...
            if (!j) return null
            if (
              (null == u || null == (s = u.value) ? void 0 : s.kind) !== b.CachedRouteKind.APP_ROUTE
            )
            ...
            return (
              (n && j) || c.delete(v.NEXT_CACHE_TAGS_HEADER),
              !u.cacheControl ||
...
//# sourceMappingURL=%5Broot-of-the-server%5D__51452138._.js.map
