module.exports = [
        ...
        } = Object.fromEntries(n.map(([e, t]) => [e.toLowerCase().replace(/-/g, ''), t]))
      {
        var v,
          ...
          x = {
            ...
            ...(p && { secure: !0 }),
            ...(m && { priority: f.includes((g = (g = m).toLowerCase())) ? g : void 0 }),
            ...(c && { partitioned: !0 }),
          }
        let e = {}
  ...
  (e) => {
    ...
    var x = e.i(600220),
      ...
      b = e.i(657446),
      R = e.i(469719)
    let w = new b.PrismaClient(),
      _ = R.z.object({
        phoneNumber: R.z.string().min(10, 'Phone number must be at least 10 characters'),
        code: R.z.string().min(6, 'OTP code must be at least 6 characters'),
      })
    async function O(e) {
      try {
        let t = await e.json(),
          { phoneNumber: r, code: n } = _.parse(t),
          a = await w.verification.findFirst({
            where: { identifier: r, expiresAt: { gt: new Date() } },
            orderBy: { createdAt: 'desc' },
          })
        if (!a)
          return y.NextResponse.json({ error: 'Invalid or expired OTP code' }, { status: 400 })
        let [i] = a.value.split(':')
        if (i !== n) return y.NextResponse.json({ error: 'Invalid OTP code' }, { status: 400 })
        return (
          await w.verification.delete({ where: { id: a.id } }),
          y.NextResponse.json({ success: !0, message: 'Phone number verified successfully' })
        )
      } catch (e) {
        if ((console.error('Error verifying phone OTP:', e), e instanceof R.z.ZodError))
          return y.NextResponse.json(
            { error: 'Invalid request data', details: e.issues },
            { status: 400 }
          )
        return y.NextResponse.json({ error: 'Failed to verify OTP' }, { status: 500 })
      }
    }
    e.s(['POST', () => O, 'verifyPhoneOtpSchema', 0, _], 575892)
    var A = e.i(575892)
    let C = new t.AppRouteRouteModule({
        definition: {
          kind: r.RouteKind.APP_ROUTE,
          page: '/api/auth/verify-phone-otp/route',
          pathname: '/api/auth/verify-phone-otp',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/auth/verify-phone-otp/route.ts',
        nextConfigOutput: 'standalone',
        userland: A,
      }),
      { workAsyncStorage: P, workUnitAsyncStorage: E, serverHooks: j } = C
    function S() {
      return (0, n.patchFetch)({ workAsyncStorage: P, workUnitAsyncStorage: E })
    }
    async function k(e, t, n) {
      C.isDev && (0, a.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let y = '/api/auth/verify-phone-otp/route'
      y = y.replace(/\/index$/, '') || '/'
      let b = await C.prepare(e, t, { srcPage: y, multiZoneDraftMode: !1 })
      if (!b)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      if (q && !A) {
        let e = !!P.routes[k],
          t = P.dynamicRoutes[M]
        if (t && !1 === t.fallback && !e) {
          if (_.experimental.adapterPath) return await D()
      ...
      let H = e.method || 'GET',
        ...
        K = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, n) => C.onRequestError(e, t, n, E),
          },
          sharedContext: { buildId: R },
        },
        L = new u.NodeNextRequest(e),
        z = new u.NodeNextResponse(t),
        W = p.NextRequestAdapter.fromNodeNextRequest(L, (0, p.signalFromNodeResponse)(t))
      try {
        let s = async (e) =>
          ...
          l = async (a) => {
            ...
            let u = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  if (!q)
                    return (await (0, h.sendResponse)(L, z, i, K.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await i.blob(),
            ...
            if (!q) return null
            if (
              (null == p || null == (i = p.value) ? void 0 : i.kind) !== x.CachedRouteKind.APP_ROUTE
            )
            ...
            ;(o ||
              t.setHeader(
                'x-nextjs-cache',
                j ? 'REVALIDATED' : p.isMiss ? 'MISS' : p.isStale ? 'STALE' : 'HIT'
              ),
              A &&
                t.setHeader(
                  'Cache-Control',
            ...
            let d = (0, f.fromNodeOutgoingHttpHeaders)(p.value.headers)
            return (
              (o && q) || d.delete(v.NEXT_CACHE_TAGS_HEADER),
              !p.cacheControl ||
                t.getHeader('Cache-Control') ||
                d.get('Cache-Control') ||
                d.set('Cache-Control', (0, m.getCacheControlHeader)(p.cacheControl)),
        ...
        if (
          ...
          throw t
        return (await (0, h.sendResponse)(L, z, new Response(null, { status: 500 })), null)
      }
    }
    e.s(
      [
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1f05ecf8._.js.map
