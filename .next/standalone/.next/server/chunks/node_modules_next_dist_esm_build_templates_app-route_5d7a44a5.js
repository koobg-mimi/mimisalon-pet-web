module.exports = [
  ...
  (e) => {
    ...
    var h = e.i(600220),
      f = e.i(89171),
      z = e.i(469719)
    let T = z.z.enum(['TEXT', 'IMAGE', 'FILE', 'BOOKING_INFO', 'SYSTEM']),
      O = z.z.enum(['CUSTOMER_GROOMER', 'CUSTOMER_ADMIN', 'GROOMER_ADMIN', 'GROUP']),
      A = z.z.enum(['SENT', 'DELIVERED', 'READ', 'FAILED'])
    z.z.object({
      conversationId: z.z.string().min(1, '대화 ID가 필요합니다'),
      content: z.z
        .string()
      ...
      replyToId: z.z.string().optional(),
    })
    let C = z.z.object({
      type: O,
      participants: z.z.array(z.z.string()).min(2, '최소 2명의 참가자가 필요합니다'),
      title: z.z.string().optional(),
      bookingId: z.z.string().optional(),
      metadata: z.z.record(z.z.string(), z.z.unknown()).optional(),
    })
    ;(z.z.object({
      conversationId: z.z.string(),
      page: z.z.number().min(1).default(1),
      limit: z.z.number().min(1).max(100).default(20),
      beforeMessageId: z.z.string().optional(),
      afterMessageId: z.z.string().optional(),
    }),
      z.z.object({
        type: O.optional(),
        page: z.z.number().min(1).default(1),
        limit: z.z.number().min(1).max(50).default(20),
        search: z.z.string().optional(),
        unreadOnly: z.z.boolean().default(!1),
      }),
      z.z.object({
        content: z.z.string().min(1).max(1e3).optional(),
        status: A.optional(),
        readAt: z.z.date().optional(),
      }),
      z.z.object({
        title: z.z.string().max(100).optional(),
        isArchived: z.z.boolean().optional(),
        isMuted: z.z.boolean().optional(),
        mutedUntil: z.z.date().optional(),
      }),
      z.z.object({
      ...
      z.z.object({
        query: z.z.string().min(1, '검색어를 입력해주세요'),
        conversationId: z.z.string().optional(),
        type: T.optional(),
        fromDate: z.z.string().optional(),
      ...
      z.z.object({
        ...
        conversationId: z.z.string(),
        userId: z.z.string(),
        messageId: z.z.string().optional(),
        data: z.z.record(z.z.string(), z.z.unknown()).optional(),
      }))
    let y = [
      {
        id: 'conv_001',
      ...
      try {
        let { searchParams: t } = new URL(e.url),
          n = {
            ...
            unreadOnly: 'true' === t.get('unreadOnly'),
          },
          a = 'user_001',
          r = y.filter((e) => e.participants.some((e) => e.id === a))
        if (
          (n.type && (r = r.filter((e) => e.type === n.type)),
          n.unreadOnly && (r = r.filter((e) => e.unreadCount > 0)),
          n.search)
        ) {
          let e = n.search.toLowerCase()
          r = r.filter((t) => {
            let n = t.participants.find((e) => e.id !== a)
            return (
              (n?.name || '').toLowerCase().includes(e) ||
              t.lastMessage?.content.toLowerCase().includes(e)
            )
          })
        }
        r.sort((e, t) => {
          let n = new Date(e.lastMessage?.createdAt || e.updatedAt).getTime()
          return new Date(t.lastMessage?.createdAt || t.updatedAt).getTime() - n
        })
        let o = (n.page - 1) * n.limit,
          i = o + n.limit,
          s = r.slice(o, i)
        return f.NextResponse.json({
          ...
          pagination: {
            ...
            limit: n.limit,
            total: r.length,
            totalPages: Math.ceil(r.length / n.limit),
          },
          summary: { unreadCount: r.filter((e) => e.unreadCount > 0).length, totalCount: r.length },
        })
      } catch (e) {
        return (
          console.error('Error fetching conversations:', e),
          f.NextResponse.json({ error: '대화 목록을 불러올 수 없습니다' }, { status: 500 })
        )
      }
    }
    async function _(e) {
      try {
        ...
        if ((console.error('Error creating conversation:', e), e instanceof z.z.ZodError))
          return f.NextResponse.json(
            { error: '잘못된 대화 데이터입니다', details: e.issues },
            { status: 400 }
          )
        return f.NextResponse.json({ error: '대화 생성 중 오류가 발생했습니다' }, { status: 500 })
      }
    }
    e.s(['GET', () => S, 'POST', () => _], 882507)
    var w = e.i(882507)
    let I = new t.AppRouteRouteModule({
      ...
      { workAsyncStorage: M, workUnitAsyncStorage: b, serverHooks: x } = I
    function N() {
      return (0, a.patchFetch)({ workAsyncStorage: M, workUnitAsyncStorage: b })
    }
    async function D(e, t, a) {
      I.isDev && (0, r.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let f = '/api/conversations/route'
      f = f.replace(/\/index$/, '') || '/'
      let z = await I.prepare(e, t, { srcPage: f, multiZoneDraftMode: !1 })
      if (!z)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
          ...
          isOnDemandRevalidate: w,
          revalidateOnlyGenerated: M,
          resolvedPathname: b,
          clientReferenceManifest: x,
          serverActionsManifest: N,
        } = z,
        D = (0, l.normalizeAppPath)(f),
        U = !!(S.dynamicRoutes[D] || S.routes[b]),
        j = async () => (
          (null == _ ? void 0 : _.render404)
            ? await _.render404(e, t, C, !1)
      ...
      N &&
        ...
        (0, i.setReferenceManifestsSingleton)({
          ...
          clientReferenceManifest: x,
          serverActionsManifest: N,
          serverModuleMap: (0, s.createServerModuleMap)({ serverActionsManifest: N }),
        })
      let H = e.method || 'GET',
        Z = (0, o.getTracer)(),
        q = Z.getActiveScopeSpan(),
        F = {
          params: O,
          prerenderManifest: S,
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, n, a) => I.onRequestError(e, t, a, _),
          },
          sharedContext: { buildId: T },
        },
        L = new d.NodeNextRequest(e),
        $ = new d.NodeNextResponse(t),
        K = u.NextRequestAdapter.fromNodeNextRequest(L, (0, u.signalFromNodeResponse)(t))
      try {
        let i = async (e) =>
            I.handle(K, F).finally(() => {
              if (!e) return
              e.setAttributes({ 'http.status_code': t.statusCode, 'next.rsc': !1 })
              let n = Z.getRootSpanAttributes()
              if (!n) return
          ...
          l = async (r) => {
            ...
            let d = async ({ previousCacheEntry: n }) => {
                try {
                  ...
                  if (!U)
                    return (await (0, g.sendResponse)(L, $, o, F.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await o.blob(),
            ...
            if (!U) return null
            if (
              (null == u || null == (o = u.value) ? void 0 : o.kind) !== h.CachedRouteKind.APP_ROUTE
            )
            ...
            ;(s ||
              t.setHeader(
                'x-nextjs-cache',
                w ? 'REVALIDATED' : u.isMiss ? 'MISS' : u.isStale ? 'STALE' : 'HIT'
              ),
              y &&
                t.setHeader(
                  'Cache-Control',
        ...
        if (
          ...
          throw t
        return (await (0, g.sendResponse)(L, $, new Response(null, { status: 500 })), null)
      }
    }
    e.s(
      [
