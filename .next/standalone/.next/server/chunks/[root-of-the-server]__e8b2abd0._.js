  ...
  (e) => {
    ...
    function n(e, r) {
      let n =
        ...
        a = (0, s.toDate)(e, r?.in),
        o = a.getDay()
      return (a.setDate(a.getDate() - (7 * (o < n) + o - n)), a.setHours(0, 0, 0, 0), a)
    }
  ...
  (e) => {
    ...
    let A = {
      ...
      bookingPets: {
        include: {
          pet: {
            include: { breed: { select: { name: !0 } }, images: { select: { url: !0 }, take: 1 } },
          },
          services: { include: { service: { select: { id: !0, name: !0, description: !0 } } } },
          selectedOptions: {
            include: { serviceOption: { select: { id: !0, name: !0, description: !0 } } },
          },
        },
      },
      payments: { select: { id: !0, amount: !0, status: !0, method: !0, createdAt: !0 } },
    }
    async function S(e, { params: t }) {
      try {
        let e = await y.default.api.getSession({ headers: await (0, w.headers)() })
        if (!e?.user || 'GROOMER' !== e.user.role)
          return b.NextResponse.json({ error: '미용사 권한이 필요합니다' }, { status: 403 })
        let { id: r } = await t,
          s = await E.prisma.booking.findFirst({
            where: { id: r, groomerId: e.user.id },
            include: A,
          })
        if (!s) return b.NextResponse.json({ error: '예약을 찾을 수 없습니다' }, { status: 404 })
        let n = {
          id: s.id,
          bookingNumber: s.bookingNumber,
          appointmentDate: (0, k.format)(s.serviceDate, 'yyyy-MM-dd', { locale: _.ko }),
          startTime: s.serviceTime,
          endTime: T(s.serviceTime, s.estimatedDurationMinutes),
          status: s.status,
          paymentStatus: s.paymentStatus,
          totalAmount: s.basePrice + s.additionalCharges - s.discountAmount,
          paidAmount: s.payments
            .filter((e) => 'PAID' === e.status || 'COMPLETED' === e.status)
            .reduce((e, t) => e + t.amount, 0),
          customer: {
            id: s.customer.id,
            name: s.customer.name || '고객',
          ...
          groomer: s.groomer
            ? {
                id: s.groomer.id,
                name: s.groomer.name || '미용사',
                phone: s.groomer.phoneNumber || '',
                email: s.groomer.email,
                photoUrl: s.groomer.image || null,
                experience: null,
              }
            : null,
          pets: s.bookingPets.map((e) => ({
            id: e.pet.id,
            name: e.pet.name,
            species: e.pet.type,
            breed: e.pet.breed?.name || e.pet.type || 'Unknown',
            age: e.pet.age || 0,
            weight: e.pet.weight || 0,
            photoUrl: e.pet.images?.[0]?.url || null,
            services: e.services.map((e) => ({
              id: e.id,
              serviceId: e.service.id,
              name: e.service.name,
              description: e.service.description || '',
              duration: e.serviceDurationMinutes || 60,
              price: e.servicePrice,
              status: s.status,
            })),
            options: e.selectedOptions.map((e) => ({
          ...
          location: s.customerAddress
            ? {
                address: `${s.customerAddress.street}, ${s.customerAddress.city}, ${s.customerAddress.state}`,
                zipCode: s.customerAddress.zipCode,
              }
            : null,
          notes: s.notes,
          ...
          estimatedEndTime: T(s.serviceTime, s.estimatedDurationMinutes),
          actualStartTime: s.actualStartTime
            ? (0, k.format)(s.actualStartTime, 'HH:mm', { locale: _.ko })
            : null,
          actualEndTime: s.actualEndTime
            ? (0, k.format)(s.actualEndTime, 'HH:mm', { locale: _.ko })
            : null,
          createdAt: s.createdAt.toISOString(),
          updatedAt: s.updatedAt.toISOString(),
        }
        return b.NextResponse.json(n)
      } catch (e) {
        return (
          console.error('Groomer booking detail fetch error:', e),
          b.NextResponse.json(
    ...
    function T(e, t) {
      let [r, s] = e.split(':').map(Number),
        n = 60 * r + s + t,
        a = Math.floor(n / 60)
      return `${String(a).padStart(2, '0')}:${String(n % 60).padStart(2, '0')}`
    }
    e.s(['GET', () => S], 257986)
    var C = e.i(257986)
    let x = new t.AppRouteRouteModule({
        definition: {
          ...
          page: '/api/groomer/bookings/[id]/route',
          pathname: '/api/groomer/bookings/[id]',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/groomer/bookings/[id]/route.ts',
        nextConfigOutput: 'standalone',
        userland: C,
      }),
      { workAsyncStorage: O, workUnitAsyncStorage: P, serverHooks: N } = x
    function D() {
      return (0, s.patchFetch)({ workAsyncStorage: O, workUnitAsyncStorage: P })
    }
    async function j(e, t, s) {
      x.isDev && (0, n.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let _ = '/api/groomer/bookings/[id]/route'
      _ = _.replace(/\/index$/, '') || '/'
      ...
      let {
        ...
        M = !!(S.dynamicRoutes[j] || S.routes[P]),
        I = async () => (
          (null == T ? void 0 : T.render404)
            ? await T.render404(e, t, k, !1)
      ...
      let $ = e.method || 'GET',
        ...
        G = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, s) => x.onRequestError(e, t, s, T),
          },
      ...
      try {
          ...
          l = async (n) => {
            ...
            let u = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  l && s.waitUntil && (s.waitUntil(l), (l = void 0))
                  let u = G.renderOpts.collectedTags
                  if (!M)
                    return (await (0, m.sendResponse)(L, B, a, G.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await a.blob(),
        ...
        if (
          ...
          throw t
        return (await (0, m.sendResponse)(L, B, new Response(null, { status: 500 })), null)
      }
    }
    e.s(
      [
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e8b2abd0._.js.map
