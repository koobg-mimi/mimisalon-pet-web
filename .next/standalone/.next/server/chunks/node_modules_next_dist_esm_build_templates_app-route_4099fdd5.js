module.exports = [
  ...
  (e) => {
    ...
    async function y(e) {
      try {
        let t = x.env.CRON_SECRET,
          a = e.headers.get('authorization')
        if (t && a !== `Bearer ${t}`)
          return v.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        let n = new Date(Date.now() - 18e5),
          r = await f.prisma.payment.updateMany({
            where: { status: 'PENDING', createdAt: { lte: n } },
            data: { status: 'EXPIRED', updatedAt: new Date() },
          })
        for (let e of await f.prisma.payment.findMany({
          where: {
            status: 'EXPIRED',
            updatedAt: { gte: new Date(Date.now() - 6e4) },
            bookingId: { not: null },
          },
          include: { booking: !0 },
        }))
          e.booking &&
            ('FIRST_PAYMENT_PENDING' === e.booking.status ||
              'ADDITIONAL_PAYMENT_PENDING' === e.booking.status) &&
            (await f.prisma.booking.update({
        ...
        return (
          console.log(`[Cron] Cleaned up ${r.count} expired payments`),
          v.NextResponse.json({
            success: !0,
            message: `Updated ${r.count} expired payments`,
        ...
        return (
          console.error('[Cron] Error cleaning up expired payments:', e),
          v.NextResponse.json({ error: 'Failed to clean up expired payments' }, { status: 500 })
        )
      }
    }
    async function C(e) {
    ...
    e.s(['GET', () => y, 'POST', () => C], 504573)
    var A = e.i(504573)
    let N = new t.AppRouteRouteModule({
        definition: {
          kind: a.RouteKind.APP_ROUTE,
          page: '/api/cron/cleanup-expired-payments/route',
          pathname: '/api/cron/cleanup-expired-payments',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/cron/cleanup-expired-payments/route.ts',
        nextConfigOutput: 'standalone',
        userland: A,
      }),
      { workAsyncStorage: P, workUnitAsyncStorage: T, serverHooks: D } = N
    function I() {
      return (0, n.patchFetch)({ workAsyncStorage: P, workUnitAsyncStorage: T })
    }
    async function b(e, t, n) {
      N.isDev && (0, r.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let v = '/api/cron/cleanup-expired-payments/route'
      v = v.replace(/\/index$/, '') || '/'
      ...
      let {
        ...
        k = (0, l.normalizeAppPath)(v),
        M = !!(T.dynamicRoutes[k] || T.routes[O]),
        U = async () => (
          (null == D ? void 0 : D.render404)
            ? await D.render404(e, t, A, !1)
      ...
      let F = e.method || 'GET',
        ...
        G = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, a, n) => N.onRequestError(e, t, n, D),
          },
          sharedContext: { buildId: x },
        },
        B = new u.NodeNextRequest(e),
        L = new u.NodeNextResponse(t),
        X = d.NextRequestAdapter.fromNodeNextRequest(B, (0, d.signalFromNodeResponse)(t))
      try {
        let i = async (e) =>
          ...
          l = async (r) => {
            ...
            let u = async ({ previousCacheEntry: a }) => {
                try {
                  ...
                  if (!M)
                    return (await (0, h.sendResponse)(B, L, o, G.renderOpts.pendingWaitUntil), null)
                  {
                    let e = await o.blob(),
                      t = (0, R.toNodeOutgoingHttpHeaders)(o.headers)
            ...
            if (!M) return null
            if (
              (null == d || null == (o = d.value) ? void 0 : o.kind) !== w.CachedRouteKind.APP_ROUTE
            )
            ...
            let p = (0, R.fromNodeOutgoingHttpHeaders)(d.value.headers)
            return (
              (s && M) || p.delete(m.NEXT_CACHE_TAGS_HEADER),
              !d.cacheControl ||
                t.getHeader('Cache-Control') ||
                p.get('Cache-Control') ||
                p.set('Cache-Control', (0, E.getCacheControlHeader)(d.cacheControl)),
...
//# sourceMappingURL=node_modules_next_dist_esm_build_templates_app-route_4099fdd5.js.map
