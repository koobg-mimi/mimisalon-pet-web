module.exports = [
  ...
  (e) => {
    ...
    var w = e.i(600220),
      ...
      k = e.i(657446)
    let b = y.z.object({
      workingDays: y.z.array(y.z.string()),
      workingHours: y.z.object({
        start: y.z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/),
        end: y.z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/),
      }),
      slotDuration: y.z.number().default(30),
    })
    async function x() {
      try {
        let e = await E.default.api.getSession({ headers: await (0, _.headers)() })
        if (!e || e.user?.role !== 'GROOMER')
          return f.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        let t = e.user.id,
          r = await k.prisma.groomerSchedule.findUnique({ where: { groomerId: t } })
        r ||
          (r = await k.prisma.groomerSchedule.create({
            data: {
              groomerId: t,
        ...
        let a = {
          workingDays: r.workingDays.map(String),
          workingHours: { start: r.workingHoursStart, end: r.workingHoursEnd },
          slotDuration: r.slotDurationMinutes,
        }
        return f.NextResponse.json(a)
      } catch (e) {
        return (
          console.error('Failed to fetch groomer settings:', e),
    ...
    async function S(e) {
      try {
        let t = await E.default.api.getSession({ headers: await (0, _.headers)() })
        if (!t || t.user?.role !== 'GROOMER')
          return f.NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        let r = t.user.id,
          a = await e.json(),
          { workingDays: n, workingHours: s, slotDuration: o } = b.parse(a)
        if (!n || !s || !s.start || !s.end || !o)
          return f.NextResponse.json({ error: 'Missing required fields' }, { status: 400 })
        let i = n.map((e) => parseInt(e))
        if (i.some((e) => e < 0 || e > 6))
          return f.NextResponse.json({ error: 'Invalid working days' }, { status: 400 })
        let l = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/
        if (!l.test(s.start) || !l.test(s.end))
          return f.NextResponse.json({ error: 'Invalid time format' }, { status: 400 })
        let u = C(s.start),
          d = C(s.end)
        if (u >= d)
        ...
        if (30 !== o)
          return f.NextResponse.json({ error: 'Slot duration must be 30 minutes' }, { status: 400 })
        let c = await k.prisma.groomerSchedule.upsert({
            where: { groomerId: r },
            update: {
              workingDays: i,
              workingHoursStart: s.start,
              workingHoursEnd: s.end,
              slotDurationMinutes: o,
            },
            create: {
              groomerId: r,
              workingDays: i,
          ...
          p = new Date()
        ;(p.setHours(0, 0, 0, 0),
          await k.prisma.groomerAvailability.deleteMany({
            where: { groomerId: r, date: { gte: p }, isBooked: !1 },
          }))
        let h = {
          workingDays: c.workingDays.map(String),
          workingHours: { start: c.workingHoursStart, end: c.workingHoursEnd },
          slotDuration: c.slotDurationMinutes,
        }
        return f.NextResponse.json(h)
      } catch (e) {
        return (
          console.error('Failed to update groomer settings:', e),
          f.NextResponse.json({ error: 'Internal server error' }, { status: 500 })
        )
      }
    }
    ...
    e.s(['GET', () => x, 'PUT', () => S, 'updateAvailabilitySettingsSchema', 0, b], 36439)
    var N = e.i(36439)
    let j = new t.AppRouteRouteModule({
        definition: {
          kind: r.RouteKind.APP_ROUTE,
          page: '/api/groomer/availability/settings/route',
          pathname: '/api/groomer/availability/settings',
          filename: 'route',
          bundlePath: '',
        },
        distDir: '.next',
        relativeProjectDir: '',
        resolvedPagePath: '[project]/src/app/api/groomer/availability/settings/route.ts',
        nextConfigOutput: 'standalone',
        userland: N,
      }),
      { workAsyncStorage: A, workUnitAsyncStorage: H, serverHooks: D } = j
    function P() {
      return (0, a.patchFetch)({ workAsyncStorage: A, workUnitAsyncStorage: H })
    }
    async function T(e, t, a) {
      j.isDev && (0, n.addRequestMeta)(e, 'devRequestTimingInternalsEnd', process.hrtime.bigint())
      let f = '/api/groomer/availability/settings/route'
      f = f.replace(/\/index$/, '') || '/'
      let _ = await j.prepare(e, t, { srcPage: f, multiZoneDraftMode: !1 })
      if (!_)
        return (
          (t.statusCode = 400),
          t.end('Bad Request'),
      ...
      let {
        ...
        T = (0, l.normalizeAppPath)(f),
        M = !!(S.dynamicRoutes[T] || S.routes[H]),
        O = async () => (
      ...
      if (M && !x) {
        let e = !!S.routes[H],
          t = S.dynamicRoutes[T]
        if (t && !1 === t.fallback && !e) {
          if (k.experimental.adapterPath) return await O()
          throw new R.NoFallbackError()
        }
      }
      let I = null
      !M || j.isDev || x || (I = '/index' === (I = H) ? '/' : I)
      let U = !0 === j.isDev || !M,
        q = M && !U
      P &&
        D &&
      ...
      let $ = e.method || 'GET',
        ...
        K = {
          ...
          renderOpts: {
            ...
            onInstrumentationRequestError: (t, r, a) => j.onRequestError(e, t, a, C),
          },
          sharedContext: { buildId: y },
        },
        B = new u.NodeNextRequest(e),
        G = new u.NodeNextResponse(t),
        L = d.NextRequestAdapter.fromNodeNextRequest(B, (0, d.signalFromNodeResponse)(t))
      try {
        let o = async (e) =>
            j.handle(L, K).finally(() => {
              if (!e) return
          ...
          l = async (n) => {
            ...
            let u = async ({ previousCacheEntry: r }) => {
                try {
                  ...
                  {
                    let e = await s.blob(),
                      t = (0, g.toNodeOutgoingHttpHeaders)(s.headers)
...
//# sourceMappingURL=%5Broot-of-the-server%5D__cdcf9785._.js.map
